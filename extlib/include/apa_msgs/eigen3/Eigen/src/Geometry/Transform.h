 ...
 *exploited to speed up some functions such as inverse() and rotation(). \tparam _Options
 *has the same meaning as in class Matrix. It allows to specify DontAlign and/or RowMajor.
...
class Transform
{
  public:
    ...
    typedef typename internal::make_proper_matrix_type<Scalar, Rows, HDim, Options>::type
        MatrixType;
  ...
  public:
    ...
    EIGEN_DEVICE_FUNC inline Transform()
    {
        ...
        internal::transform_make_affine<(int(Mode) == Affine || int(Mode) == Isometry)
                                            ? Affine
    ...
    /** \returns an expression of the product between the transform \c *this and a matrix
     * expression \a other.
     *
     ...
     *
     * Moreover, if \c *this represents an affine transformation (i.e., Mode!=Projective),
     * then \a other can also be: \li a point of size Dim (computes: \code this->linear()
     * * other + this->translation()\endcode), \li a set of N points as a Dim x N matrix
     * (computes: \code (this->linear() * other).colwise() + this->translation()\endcode),
     *
     * In all cases, the return type is a matrix or vector of same sizes as the
     * right-hand-side \a other.
     *
     * If you want to interpret \a other as a linear or affine transformation, then first
     * convert it to a Transform<> type, or do your own cooking.
     *
    ...
    /** \returns the product expression of a transformation matrix \a a times a transform
     * \a b
     *
    ...
    {
        return internal::transform_left_product_impl<OtherDerived, Mode, Options, Dim,
  ...
  private:
    ...
    //  (the meaning of a name may have changed since the template declaration -- the type
    //  of the template is:
    // "Eigen::internal::transform_transform_product_impl<Eigen::Transform<double, 3, 32,
    // 0>,
  ...
  public:
    ...
    {
        typedef
            typename icc_11_workaround<OtherMode, OtherOptions>::ProductType ProductType;
     ...
     */
    EIGEN_DEVICE_FUNC inline Block<MatrixType, int(Mode) == int(Projective) ? HDim : Dim,
                                   Dim>
    linearExt()
    {
...
template <typename Derived>
EIGEN_DEVICE_FUNC inline Transform<Scalar, Dim, Mode, Options> Transform<
    Scalar, Dim, Mode, Options>::operator*(const RotationBase<Derived, Dim> &r) const
{
    Transform res = *this;
...
{
    template <typename TransformType>
    EIGEN_DEVICE_FUNC static inline const typename TransformType::LinearMatrixType run(
...
/** decomposes the linear part of the transformation as a product rotation x scaling, the
 * scaling being not necessarily positive.
 *
...
/** decomposes the linear part of the transformation as a product scaling x rotation, the
 * scaling being not necessarily positive.
 *
 * If either pointer is zero, the corresponding computation is skipped.
 *
...
EIGEN_DEVICE_FUNC Transform<Scalar, Dim, Mode, Options> Transform<
    Scalar, Dim, Mode, Options>::inverse(TransformTraits hint) const
{
    Transform res;
    if (hint == Projective)
    {
...
struct transform_construct_from_matrix<Other, AffineCompact, Options, Dim, HDim, HDim,
                                       HDim>
{
    static inline void run(
...
struct transform_right_product_impl<TransformType, MatrixType, 0, RhsCols>
{
    ...
    static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ResultType run(const TransformType &T,
