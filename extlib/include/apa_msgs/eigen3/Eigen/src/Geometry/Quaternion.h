...
class QuaternionBase : public RotationBase<Derived, 3>
{
  public:
    ...
    typedef
        typename internal::conditional<bool(internal::traits<Derived>::Flags &LvalueBit),
                                       Scalar &, CoeffReturnType>::type
    ...
    // compiling test_stdvector with GCC 4.4.2. This looks like a GCC bug though, so feel
    // free to re-enable it if it's useful; however notice that we already have the
    // templated operator= above and e.g. in MatrixBase we didn't have to add, in addition
    // to templated operator=, such a non-templated copy operator.
    ...
    template <typename NewScalarType>
    EIGEN_DEVICE_FUNC inline
        typename internal::enable_if<internal::is_same<Scalar, NewScalarType>::value,
                                     const Derived &>::type
        cast() const
    {
        return derived();
 ...
 * \tparam _Options controls the memory alignment of the coefficients. Can be \# AutoAlign
 * or \# DontAlign. Default is AutoAlign.
 *
 ...
 *
 * \warning Operations interpreting the quaternion as rotation have undefined behavior if
...
class Quaternion : public QuaternionBase<Quaternion<_Scalar, _Options>>
{
  public:
     ...
     * [\c x, \c y, \c z, \c w]
     */
    EIGEN_DEVICE_FUNC inline Quaternion(const Scalar &w, const Scalar &x, const Scalar &y,
                                        const Scalar &z)
        : m_coeffs(x, y, z, w)
    {
    }
    ...
    // We define a copy constructor, which means we don't get an implicit move constructor
    ...
    : public QuaternionBase<Map<const Quaternion<_Scalar>, _Options>>
{
  public:
     ...
     *
     * If the template parameter _Options is set to #Aligned, then the pointer coeffs must
     * be aligned. */
 ...
 */
template <class Derived>
EIGEN_DEVICE_FUNC inline typename QuaternionBase<Derived>::Matrix3 QuaternionBase<
    Derived>::toRotationMatrix(void) const
{
...
template <class Derived>
template <class OtherDerived>
EIGEN_DEVICE_FUNC inline typename internal::traits<Derived>::Scalar QuaternionBase<
...
template <class Derived>
template <class OtherDerived>
EIGEN_DEVICE_FUNC Quaternion<typename internal::traits<Derived>::Scalar> QuaternionBase<
    Derived>::slerp(const Scalar &t, const QuaternionBase<OtherDerived> &other) const
{
