                                           ...
                                           Index num_threads = 1)
{
    ...
    // Explanations:
    // Let's recall that the product algorithms form mc x kc vertical panels A' on the lhs
    // and kc x nc blocks B' on the rhs. B' has to fit into L2/L3 cache. Moreover, A' is
    // processed per mr x kc horizontal small panels where mr is the blocking size along
    // the m dimension at the register level. This small horizontal panel has to stay
    // within L1 cache.
    ...
    if (num_threads > 1)
    {
        ...
        if (l3 > l2)
        {
            // l3 is shared between all cores, so we'll give each thread its own chunk of
            // l3.
    ...
    else
    {
        ...
        // of size mr x kc of the lhs plus a vertical panel of kc x nr of the rhs both
        // fits within L1 cache. We also include a register-level block of the result (mx
        // x nr). (In an ideal world only the lhs panel would stay in L1) Moreover, kc has
        // to be a multiple of 8 to be compatible with loop peeling, leading to a maximum
        // blocking size of:
        const Index max_kc =
...
// rather than overestimating it) For instance, it corresponds to 6MB of L3 shared among 4
// cores.
#ifdef EIGEN_DEBUG_SMALL_PRODUCT_BLOCKS
        ...
        // x1.5. However, if the entire lhs block fit within L1, then we are not going to
        // block on the rows at all, and it becomes fruitful to keep the packed rhs blocks
        ...
        if (n > nc)
        {
            ...
            //    Here we allow one more sweep if this gives us a perfect match, thus the
            //    commented "-1"
            n = (n % nc) == 0 ? nc
...
struct RhsPanelHelper
{
  ...
  public:
    typedef
        typename conditional<remaining_registers >= 4, RhsPacketx4, RhsPacket>::type type;
};
 ...
 *
 *  cd*cd : unpack rhs to (b_r,b_r), (b_i,b_i), mul to get (a_r b_r,a_i b_r) (a_r b_i,a_i
 * b_i), storing each res packet into two packets (2x2), at the end combine them: swap the
 * second and addsub them cf*cf : same but with 2x4 blocks cplx*real : unpack rhs to
 * constant packets, ... real*cplx : load lhs as (a0,a0,a1,a1), and mul as usual
 */
template <typename _LhsScalar, typename _RhsScalar, bool _ConjLhs, bool _ConjRhs,
          int Arch, int _PacketSize>
class gebp_traits
{
  public:
                  ...
                  _PacketSize>
{
  public:
    ...
    template <typename LhsPacketType, typename RhsPacketType, typename AccPacketType>
    EIGEN_STRONG_INLINE void madd_impl(const LhsPacketType &a, const RhsPacketType &b,
                                       AccPacketType &c, RhsPacketType &tmp,
                                       const true_type &) const
    {
#ifdef EIGEN_HAS_SINGLE_INSTRUCTION_MADD
    ...
    EIGEN_STRONG_INLINE void acc(const AccPacketType &c, const ResPacketType &alpha,
                  ...
                  Arch, _PacketSize>
{
  public:
    ...
    typedef
        typename conditional<Vectorizable, ScalarPacket, Scalar>::type LhsPacket4Packing;
    ...
    template <typename LhsPacketType, typename RhsPacketType, typename ResPacketType,
    ...
    EIGEN_STRONG_INLINE void acc(const DoublePacket<RealPacketType> &c,
                                 const ResPacketType &alpha, ResPacketType &r) const
    {
        // assemble c
        ResPacketType tmp;
        if ((!ConjLhs) && (!ConjRhs))
 ...
 */
template <typename LhsScalar, typename RhsScalar, typename Index, typename DataMapper,
          int mr, int nr, bool ConjugateLhs, bool ConjugateRhs>
struct gebp_kernel
{
...
struct last_row_process_16_packets
{
    ...
    EIGEN_STRONG_INLINE void operator()(const DataMapper &res, SwappedTraits &straits,
                                        const LhsScalar *blA, const RhsScalar *blB,
                                        Index depth, const Index endk, Index i, Index j2,
...
template <typename LhsScalar, typename RhsScalar, typename Index, typename DataMapper,
          int mr, int nr, bool ConjugateLhs, bool ConjugateRhs>
struct last_row_process_16_packets<LhsScalar, RhsScalar, Index, DataMapper, mr, nr,
                                   ConjugateLhs, ConjugateRhs, 16>
{
    typedef gebp_traits<LhsScalar, RhsScalar, ConjugateLhs, ConjugateRhs,
                        Architecture::Target>
        Traits;
    ...
    EIGEN_STRONG_INLINE void operator()(const DataMapper &res, SwappedTraits &straits,
                                        ...
                                        ResScalar alpha, SAccPacket &C0)
    {
        typedef typename unpacket_traits<typename unpacket_traits<SResPacket>::half>::half
            SResPacketQuarter;
        typedef typename unpacket_traits<typename unpacket_traits<SLhsPacket>::half>::half
            SLhsPacketQuarter;
        typedef typename unpacket_traits<typename unpacket_traits<SRhsPacket>::half>::half
            SRhsPacketQuarter;
        typedef typename unpacket_traits<typename unpacket_traits<SAccPacket>::half>::half
            SAccPacketQuarter;
...
template <int nr, Index LhsProgress, Index RhsProgress, typename LhsScalar,
          ...
          typename RhsPacket, typename ResPacket, typename GEBPTraits,
...
struct lhs_process_one_packet
{
    ...
    EIGEN_STRONG_INLINE void peeled_kc_onestep(Index K, const LhsScalar *blA,
                                               ...
                                               LhsPacket *A0, RhsPacketx4 *rhs_panel,
                                               RhsPacket *T0, AccPacket *C0,
                                               AccPacket *C1, AccPacket *C2,
                                               AccPacket *C3)
    {
    ...
    EIGEN_STRONG_INLINE void operator()(const DataMapper &res, const LhsScalar *blockA,
                                        const RhsScalar *blockB, ResScalar alpha,
                                        Index peelStart, Index peelEnd, Index strideA,
                                        Index strideB, Index offsetA, Index offsetB,
                                        int prefetch_res_offset, Index peeled_kc,
                                        Index pk, Index cols, Index depth,
                                        Index packet_cols4)
    {
        GEBPTraits traits;
...
struct lhs_process_fraction_of_packet
    : lhs_process_one_packet<nr, LhsProgress, RhsProgress, LhsScalar, RhsScalar,
                             ResScalar, AccPacket, LhsPacket, RhsPacket, ResPacket,
                              ...
                              Index offsetA, Index offsetB)
{
    ...
    {
        // Here, the general idea is to loop on each largest micro horizontal panel of the
        // lhs (3*Traits::LhsProgress x depth) and on each largest micro vertical panel of
        // the rhs (depth * nr). Blocking sizes, i.e., 'depth' has been computed so that
        // the micro horizontal panel of the lhs fit in L1. However, if depth is too
        // small, we can extend the number of rows of these horizontal panels. This actual
        // number of rows is computed as follow:
        const Index l1 =
            defaultL1CacheSize; // in Bytes, TODO, l1 should be passed to this function.
        ...
        // our known l1 cache size is inaccurate (e.g. on Android, we can only guess), or
        // because we are testing specific blocking sizes.
...
#undef EIGEN_GEBGP_ONESTEP
    ...
    if (mr >= 1 * Traits::LhsProgress)
    {
        lhs_process_one_packet<nr, LhsProgress, RhsProgress, LhsScalar, RhsScalar,
                               ResScalar, AccPacket, LhsPacket, RhsPacket, ResPacket,
                               Traits, LinearMapper, DataMapper>
            p;
        p(res, blockA, blockB, alpha, peeled_mc2, peeled_mc1, strideA, strideB, offsetA,
          offsetB, prefetch_res_offset, peeled_kc, pk, cols, depth, packet_cols4);
    }
    //---------- Process LhsProgressHalf rows at once ----------
    if ((LhsProgressHalf < LhsProgress) && mr >= LhsProgressHalf)
    {
        lhs_process_fraction_of_packet<nr, LhsProgressHalf, RhsProgressHalf, LhsScalar,
                                       RhsScalar, ResScalar, AccPacketHalf, LhsPacketHalf,
                                       RhsPacketHalf, ResPacketHalf, HalfTraits,
    ...
    if ((LhsProgressQuarter < LhsProgressHalf) && mr >= LhsProgressQuarter)
    {
        lhs_process_fraction_of_packet<
            nr, LhsProgressQuarter, RhsProgressQuarter, LhsScalar, RhsScalar, ResScalar,
            AccPacketQuarter, LhsPacketQuarter, RhsPacketQuarter, ResPacketQuarter,
            QuarterTraits, LinearMapper, DataMapper>
            p;
        p(res, blockA, blockB, alpha, peeled_mc_half, peeled_mc_quarter, strideA, strideB,
          offsetA, offsetB, prefetch_res_offset, peeled_kc, pk, cols, depth,
          packet_cols4);
    }
            ...
            for (Index i = peeled_mc_quarter; i < rows; i += 1)
            {
                    ...
                    (SwappedTraits::LhsProgress != 16 || SResPacketQuarterSize == nr))
                {
                    ...
                    for (; k < endk4; k += 4 * spk)
                    {
                        ...
                        straits.loadLhsUnaligned(blB + 0 * SwappedTraits::LhsProgress,
                                                 A0);
                        straits.loadLhsUnaligned(blB + 1 * SwappedTraits::LhsProgress,
                                                 A1);
                        ...
                        straits.loadLhsUnaligned(blB + 2 * SwappedTraits::LhsProgress,
                                                 A0);
                        straits.loadLhsUnaligned(blB + 3 * SwappedTraits::LhsProgress,
                                                 A1);
                    ...
                    if (SwappedTraits::LhsProgress == 8)
                    {
                        ...
                        typedef typename conditional<
                            SwappedTraits::LhsProgress >= 8,
                            typename unpacket_traits<SRhsPacket>::half, SRhsPacket>::type
                            SRhsPacketHalf;
                        typedef typename conditional<
                            SwappedTraits::LhsProgress >= 8,
...
struct gemm_pack_lhs<Scalar, Index, DataMapper, Pack1, Pack2, Packet, ColMajor, Conjugate,
                     PanelMode>
{
    typedef typename DataMapper::LinearMapper LinearMapper;
    EIGEN_DONT_INLINE void operator()(Scalar *blockA, const DataMapper &lhs, Index depth,
                                      Index rows, Index stride = 0, Index offset = 0);
};

template <typename Scalar, typename Index, typename DataMapper, int Pack1, int Pack2,
          typename Packet, bool Conjugate, bool PanelMode>
EIGEN_DONT_INLINE void gemm_pack_lhs<Scalar, Index, DataMapper, Pack1, Pack2, Packet,
                                     ColMajor, Conjugate,
                                                            ...
                                                            Index stride, Index offset)
{
    ...
    const Index peeled_mc_half =
        Pack1 >= HalfPacketSize
            ...
            : 0;
    const Index peeled_mc_quarter =
        Pack1 >= QuarterPacketSize ? (rows / (QuarterPacketSize)) * (QuarterPacketSize)
                                   : 0;
...
template <typename Scalar, typename Index, typename DataMapper, int Pack1, int Pack2,
          typename Packet, bool Conjugate, bool PanelMode>
struct gemm_pack_lhs<Scalar, Index, DataMapper, Pack1, Pack2, Packet, RowMajor, Conjugate,
                     PanelMode>
{
    typedef typename DataMapper::LinearMapper LinearMapper;
    EIGEN_DONT_INLINE void operator()(Scalar *blockA, const DataMapper &lhs, Index depth,
...
struct gemm_pack_rhs<Scalar, Index, DataMapper, nr, ColMajor, Conjugate, PanelMode>
{
    ...
    EIGEN_DONT_INLINE void operator()(Scalar *blockB, const DataMapper &rhs, Index depth,
