...
struct bit_and
{
    EIGEN_DEVICE_FUNC EIGEN_CONSTEXPR EIGEN_ALWAYS_INLINE T operator()(const T &a,
                                                                       const T &b) const
    {
        return a & b;
    }
...
struct pselect_impl
{
    static EIGEN_DEVICE_FUNC inline Packet run(const Packet &mask, const Packet &a,
                                               const Packet &b)
    {
...
/** \internal \returns the sum of the elements of upper and lower half of \a a if \a a is
 * larger than 4. For a packet {a0, a1, a2, a3, a4, a5, a6, a7}, it returns a half packet
 * {a0+a4, a1+a5, a2+a6, a3+a7} For packet-size smaller or equal to 4, this boils down to
 * a noop.
 */
template <typename Packet>
EIGEN_DEVICE_FUNC inline
...
/** \internal copy a packet with constant coefficient \a a (e.g., [a,a,a,a]) to \a *to. \a
 * to must be 16 bytes aligned */
// NOTE: this function must really be templated on the packet type (think about different
// packet types for the same scalar type)
template <typename Packet>
