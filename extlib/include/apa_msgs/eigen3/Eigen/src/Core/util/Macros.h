...
/// \internal EIGEN_COMP_CLANG set to major+minor version (e.g., 307 for clang 3.7) if the
/// compiler is clang
...
/// \internal EIGEN_COMP_ICC set to __INTEL_COMPILER if the compiler is Intel compiler, 0
/// otherwise
...
// kernels EIGEN_CUDACC implies the CUDA compiler and is used to tweak Eigen code for use
// in CUDA kernels
//
// In most cases the same tweaks are required to the Eigen code to enable in both the HIP
// and CUDA kernels. For those cases, the corresponding code should be guarded with
...
// EIGEN_USE_SYCL is a user-defined macro while __SYCL_DEVICE_ONLY__ is a compiler-defined
// macro. In most cases we want to check if both macros are defined which can be done
...
#include <cmath>
    ...
    ((defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901)) ||                   \
     ...
     (defined(_LIBCPP_VERSION) && !defined(_MSC_VER)) || (EIGEN_COMP_MSVC >= 1900) || \
...
// and adding every possible attribute to maximize inlining. This should only be used when
// really necessary: in particular, it uses __attribute__((always_inline)) on GCC, which
// most of the time is useless and can severely harm compile times.
...
//  - static is not very good because it prevents definitions from different object files
//  to be merged.
//           So static causes the resulting linked executable to be bloated with multiple
//           copies of the same function.
//  - inline is not perfect either as it unwantedly hints the compiler toward inlining the
//  function.
...
inline void assert_fail(const char *condition, const char *function, const char *file,
                        int line)
{
 ...
 * This is necessary, because the implicit definition is deprecated if the copy-assignment
 * is overridden.
 */
#if EIGEN_HAS_CXX11
 ...
 * This is necessary, because the implicitly defined assignment operator gets deleted when
 * a custom operator= is defined. With C++11 or later this also default-implements the
 * copy-constructor
 */
...
// FIXME Maybe the EIGEN_DENSE_PUBLIC_INTERFACE could be removed as importing PacketScalar
// is rarely needed
...
// EIGEN_SIZE_MIN_PREFER_DYNAMIC gives the min between compile-time sizes. 0 has absolute
// priority, followed by 1, followed by Dynamic, followed by other finite values. The
// reason for giving Dynamic the priority over finite values is that min(3, Dynamic)
// should be Dynamic, since that could be anything between 0 and 3.
...
// MaxSizes. The difference is that finite values now have priority over Dynamic, so that
// min(3, Dynamic) gives 3. Indeed, whatever the actual value is (between 0 and 3), it is
// not more than 3.
...
#define EIGEN_SCALAR_BINARY_SUPPORTED(OPNAME, TYPEA, TYPEB)                             \
    (Eigen::internal::has_ReturnType<Eigen::ScalarBinaryOpTraits<                       \
         TYPEA, TYPEB,                                                                  \
         EIGEN_CAT(EIGEN_CAT(Eigen::internal::scalar_, OPNAME), _op) < TYPEA, TYPEB>> > \
     ::value)
...
// The all function is used to enable a variadic version of eigen_assert which can take a
// parameter pack as its input.
namespace Eigen
{
namespace internal
{
