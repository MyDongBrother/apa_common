                                                            ...
                                                            Packet &exponent)
{
    ...
    exponent_offset = pselect(is_denormal, padd(exponent_offset, normalization_offset),
                              exponent_offset);
...
// Assumes IEEE floating point format.
template <typename Packet>
EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Packet pldexp_generic(const Packet &a,
                                                            const Packet &exponent)
{
    ...
    const PacketI e =
        pcast<Packet, PacketI>(pmin(pmax(exponent, pnegate(max_exponent)), max_exponent));
...
struct pldexp_fast_impl
{
    ...
    static EIGEN_STRONG_INLINE EIGEN_DEVICE_FUNC Packet run(const Packet &a,
                                                            const Packet &exponent)
    {
        const Packet bias = pset1<Packet>(
            Scalar((ScalarI(1) << (int(ExponentBits) - 1)) - ScalarI(1))); // 127
        const Packet limit =
            pset1<Packet>(Scalar((ScalarI(1) << int(ExponentBits)) - ScalarI(1))); // 255
...
// using "Extended precision modular arithmetic"
    ...
    x = pmadd(y, pset1<Packet>(5.5644315544167710640977020375430583953857421875e-11),
              x); // = 0x2e74b9ee
...
// The coefficients are: 0xbfc90f80, 0xb7354480, 0x2e74b9ee
    ...
    Packet sign_bit =
        ComputeSine ? pxor(_x, preinterpret<Packet>(plogical_shift_left<30>(y_int)))
    ...
    //    c = (A'*diag(w)*A)\(A'*diag(w)*(sin(x)-x)); # weighted LS, linear coeff forced
    //    to 1 printf('%.64f\n %.64f\n%.64f\n', c(3), c(2), c(1))
    //
...
template <typename Packet>
EIGEN_STRONG_INLINE void fast_twosum(const Packet &x, const Packet &y, Packet &s_hi,
                                     Packet &s_lo)
{
    s_hi           = padd(x, y);
...
EIGEN_STRONG_INLINE void twosum(const Packet &x_hi, const Packet &x_lo,
                                const Packet &y_hi, const Packet &y_lo, Packet &s_hi,
                                Packet &s_lo)
{
...
EIGEN_STRONG_INLINE void twoprod(const Packet &x_hi, const Packet &x_lo,
                                 const Packet &y_hi, const Packet &y_lo, Packet &p_hi,
                                 Packet &p_lo)
{
...
struct fast_accurate_exp2<float>
{
    ...
    EIGEN_STRONG_INLINE Packet operator()(const Packet &x)
    {
        ...
        // Q(x) = 1 + x * (C + x * P(x)), where the degree 4 polynomial P(x) is evaluated
        // in single precision, and the remaining steps are evaluated with extra precision
        // using double word arithmetic. C is an extra precise constant stored as a double
        // word.
        //
        // The polynomial coefficients were calculated using Sollya commands:
        // > n = 6;
...
struct fast_accurate_exp2<double>
{
    ...
    EIGEN_STRONG_INLINE Packet operator()(const Packet &x)
    {
        ...
        // Q(x) = 1 + x * (C + x * P(x)), where the degree 8 polynomial P(x) is evaluated
        // in single precision, and the remaining steps are evaluated with extra precision
        // using double word arithmetic. C is an extra precise constant stored as a double
        // word.
        //
