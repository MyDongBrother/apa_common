...
#include EIGEN_MAPBASE_PLUGIN
  ...
  protected:
    ...
    EIGEN_DEVICE_FUNC void checkSanity(
        typename internal::enable_if<(internal::traits<T>::Alignment > 0), void *>::type =
            0) const
    {
#if EIGEN_MAX_ALIGN_BYTES > 0
        // innerStride() is not set yet when this function is called, so we optimistically
        // assume the lowest plausible value:
 ...
 * It inherits MapBase<Derived, ReadOnlyAccessors> which defines the const variant for
...
class MapBase<Derived, WriteAccessors> : public MapBase<Derived, ReadOnlyAccessors>
{
  ...
  public:
    ...
    typedef typename internal::conditional<internal::is_lvalue<Derived>::value, Scalar,
