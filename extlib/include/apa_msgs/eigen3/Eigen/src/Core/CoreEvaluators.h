...
//  - scalar type
//  - etc.
// Therefore, we need specialization of evaluator providing additional template arguments
// for each kind of evaluators. We currently distinguish the following kind of evaluators:
...
struct evaluator_traits_base
{
    ...
    typedef typename storage_kind_to_evaluator_kind<typename traits<T>::StorageKind>::Kind
        Kind;
...
{
    // TODO that's not very nice to have to propagate all these traits. They are currently
    // only needed to handle outer,inner indices.
...
// -------------------- CwiseNullaryOp --------------------
// Like Matrix and Array, this is not really a unary expression, so we directly specialize
// evaluator. Likewise, there is not need to more sophisticated dispatching here.
...
struct nullary_wrapper
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T packetOp(const NullaryOp &op, IndexType i,
                                                     IndexType j) const
    {
...
struct nullary_wrapper<Scalar, NullaryOp, true, false, false>
{
    ...
    template <typename T, typename IndexType>
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T packetOp(const NullaryOp &op, IndexType = 0,
                                                     IndexType = 0) const
    {
...
// vector, for instance, using linspace and assigning a RowVectorXd to a MatrixXd or even
// a row of a MatrixXd. In this case, i==0 and j is used for the actual iteration.
template <typename Scalar, typename NullaryOp>
    ...
    : evaluator_base<CwiseNullaryOp<NullaryOp, PlainObjectType>>
{
    ...
    template <typename IndexType>
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE CoeffReturnType coeff(IndexType row,
                                                                IndexType col) const
    {
...
template <typename TernaryOp, typename Arg1, typename Arg2, typename Arg3>
struct ternary_evaluator<CwiseTernaryOp<TernaryOp, Arg1, Arg2, Arg3>, IndexBased,
                         IndexBased>
    : evaluator_base<CwiseTernaryOp<TernaryOp, Arg1, Arg2, Arg3>>
{
    ...
    {
        CoeffReadCost =
            int(evaluator<Arg1>::CoeffReadCost) + int(evaluator<Arg2>::CoeffReadCost) +
...
struct mapbase_evaluator : evaluator_base<Derived>
{
    ...
    EIGEN_STRONG_INLINE PacketType packet(Index index) const
    {
        return internal::ploadt<PacketType, LoadMode>(m_data +
                                                      index * m_innerStride.value());
    }
          ...
          typename Block<ArgType, BlockRows, BlockCols, InnerPanel>::PlainObject>
{
    ...
    {
        // TODO: for the 3.3 release, this should be turned to an internal assertion, but
        // let's keep it as is for the beta lifetime
    ...
    : evaluator_base<Reverse<ArgType, Direction>>
{
  ...
  protected:
    ...
    // If we do not reverse rows, then we do not need to know the number of rows; same for
    // columns Nonetheless, in this case it is important to set to 1 such that the
