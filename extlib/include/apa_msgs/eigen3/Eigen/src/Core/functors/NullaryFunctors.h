...
struct scalar_identity_op
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE const Scalar operator()(IndexType row,
                                                                  IndexType col) const
    {
...
struct linspaced_op_impl<Scalar, /*IsInteger*/ true>
{
    EIGEN_DEVICE_FUNC linspaced_op_impl(const Scalar &low, const Scalar &high,
        ...
        : m_low(low),
          ...
          m_divisor(
              convert_index<Scalar>((high >= low ? num_steps : -num_steps) +
                                    (high - low)) /
...
// Linear access is automatically determined from the operator() prototypes available for
// the given functor. If it exposes an operator()(i,j), then we assume the i and j
// coefficients are required independently and linear access is not possible. In all other
// cases, linear access is enabled. Users should not have to deal with this structure.
