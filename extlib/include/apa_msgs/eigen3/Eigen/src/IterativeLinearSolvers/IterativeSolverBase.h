...
class generic_matrix_wrapper<MatrixType, false>
{
  public:
    ...
    {
        typedef
            typename ActualMatrixType::template ConstSelfAdjointViewReturnType<UpLo>::Type
                Type;
    };
...
class IterativeSolverBase : public SparseSolverBase<Derived>
{
  ...
  public:
     ...
     *
     * Currently, this function mostly calls analyzePattern on the preconditioner. In the
     * future we might, for instance, implement column reordering for faster matrix vector
     * products.
     */
    template <typename MatrixDerived>
    ...
    /** Initializes the iterative solver with the numerical values of the matrix \a A for
     * further solving \c Ax=b problems.
     *
    ...
    /** Initializes the iterative solver with the matrix \a A for further solving \c Ax=b
     * problems.
     *
    ...
    void _solve_with_guess_impl(const Rhs &b, SparseMatrixBase<DestDerived> &aDest) const
    {
        ...
        // aliasing issue. For non square least-square problems, b and dest might not have
        // the same size whereas they might alias each-other.
    ...
    _solve_with_guess_impl(const Rhs &b, MatrixBase<DestDerived> &aDest) const
    {
        ...
        for (Index k = 0; k < rhsCols; ++k)
        {
            ...
            // The call to _solve_vector_with_guess updates m_info, so if it failed for a
            // previous column we need to restore it to the worst value.
            if (m_info == NumericalIssue)
                global_info = NumericalIssue;
            else if (m_info == NoConvergence)
                global_info = NoConvergence;
