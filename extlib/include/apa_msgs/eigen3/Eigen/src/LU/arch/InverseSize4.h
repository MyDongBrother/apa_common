...
// PacketMath. inv(M) = M#/|M|, where inv(M), M# and |M| denote the inverse of M, adjugate
// of M and determinant of M respectively. M# is computed block-wise using specific
// formulae. For proof, see:
...
struct compute_inverse_size4<Architecture::Target, float, MatrixType, ResultType>
{
    ...
    typedef
        typename conditional<(MatrixType::Flags & LinearAccessBit), MatrixType const &,
    ...
    static void run(const MatrixType &mat, ResultType &result)
    {
        ...
        DC = psub(
            DC, pmul(vec4f_swizzle2(D, D, 1, 1, 2, 2), vec4f_swizzle2(C, C, 2, 3, 0, 1)));
...
struct compute_inverse_size4<Architecture::Target, double, MatrixType, ResultType>
{
    ...
    static void run(const MatrixType &mat, ResultType &result)
    {
        ...
        const double sign_mask2[2] = {numext::bit_cast<double>(0x8000000000000000ull),
                                      0.0};
