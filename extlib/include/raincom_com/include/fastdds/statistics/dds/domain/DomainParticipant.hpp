...
/**
 * Class DomainParticipant: extends standard DDS DomainParticipant class to include
 * specific methods for the Statistics module
...
class DomainParticipant : public eprosima::fastdds::dds::DomainParticipant
{
  ...
  public:
     ...
     * @return RETCODE_UNSUPPORTED if the FASTDDS_STATISTICS CMake option has not been
     * set, RETCODE_BAD_PARAMETER if the topic name provided does not correspond to any
     * Statistics DataWriter, RETCODE_INCONSISTENT_POLICY if the DataWriterQos provided is
     * inconsistent, RETCODE_OK if the DataWriter has been created or if it has been
     * created previously, and RETCODE_ERROR otherwise
     */
    RTPS_DllAPI ReturnCode_t
    enable_statistics_datawriter(const std::string &topic_name,
     ...
     * @return RETCODE_UNSUPPORTED if the FASTDDS_STATISTICS CMake option has not been
     * set, RETCODE_BAD_PARAMETER if the topic name provided does not correspond to any
     * Statistics DataWriter, RETCODE_OK if the DataWriter has been correctly deleted or
     * does not exist, and RETCODE_ERROR otherwise
     */
