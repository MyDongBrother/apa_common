...
/**
 * Class DataReaderListener, it should be used by the end user to implement specific
 * callbacks to certain actions.
...
class DataReaderListener
{
  public:
     ...
     */
    RTPS_DllAPI virtual void on_data_available(DataReader *reader) { (void)reader; }

    /**
     * Virtual method to be called when the subscriber is matched with a new Writer (or
     * unmatched); i.e., when a writer publishing in the same topic is discovered.
     *
     * @param reader DataReader
     * @param info The subscription matched status
     */
    RTPS_DllAPI virtual void on_subscription_matched(
        DataReader *reader, const fastdds::dds::SubscriptionMatchedStatus &info)
    {
        (void)reader;
        (void)info;
    }
    ...
    RTPS_DllAPI virtual void on_requested_deadline_missed(
        DataReader *reader, const fastrtps::RequestedDeadlineMissedStatus &status)
    {
        (void)reader;
        (void)status;
    }
    ...
    RTPS_DllAPI virtual void on_liveliness_changed(
        DataReader *reader, const fastrtps::LivelinessChangedStatus &status)
    {
        (void)reader;
        (void)status;
    }
    ...
    RTPS_DllAPI virtual void on_sample_rejected(
        DataReader *reader, const fastrtps::SampleRejectedStatus &status)
    {
        (void)reader;
        (void)status;
    }
