...
/**
 * Class Subscriber, contains the public API that allows the user to control the reception
 * of messages. This class should not be instantiated directly. DomainRTPSParticipant
 * class should be used to correctly create this element.
 *
...
class Subscriber : public DomainEntity
{
  protected:
     ...
     * Don't use directly, create Subscriber using create_subscriber from
     * DomainParticipant.
     */
    Subscriber(SubscriberImpl *pimpl, const StatusMask &mask = StatusMask::all());

    Subscriber(DomainParticipant *dp, const SubscriberQos &qos = SUBSCRIBER_QOS_DEFAULT,
  ...
  public:
     ...
     * participant creating this Subscriber is not enabled.
     */
    ...
    /**
     * This operation creates a DataReader. The returned DataReader will be attached and
     ...
     */
    RTPS_DllAPI DataReader *create_datareader(TopicDescription *topic,
     ...
     *
     * The delete_datareader operation must be called on the same Subscriber object used
     * to create the DataReader. If delete_datareader is called on a different Subscriber,
     * the operation will have no effect and it will return an error.
     *
     * @param reader DataReader to delete
     * @return RETCODE_PRECONDITION_NOT_MET if the datareader does not belong to this
     * subscriber, RETCODE_OK if it is correctly deleted and RETCODE_ERROR otherwise.
     */
    RTPS_DllAPI ReturnCode_t delete_datareader(const DataReader *reader);

    /**
     * This operation retrieves a previously-created DataReader belonging to the
     * Subscriber that is attached to a Topic with a matching topic_name. If no such
     * DataReader exists, the operation will return nullptr.
     *
     * If multiple DataReaders attached to the Subscriber satisfy this condition, then the
     ...
     * @return Pointer to a previously created DataReader created on a Topic with that
     * topic_name
     */
    ...
    /**
     * @brief This operation allows the application to access the DataReader objects that
     * contain samples with the specified sample_states, view_states, and instance_states.
     *
     * @param[out] readers Vector of DataReader where the list of existing readers is
     * returned
    ...
    get_datareaders(std::vector<DataReader *> &readers,
    ...
    /**
     * @brief Indicates that the application is about to access the data samples in any of
    ...
    /**
     * @brief Indicates that the application has finished accessing the data samples in
    ...
    /**
     * This operation invokes the operation on_data_available on the DataReaderListener
     * objects attached to contained DataReader entities.
     *
     * This operation is typically invoked from the on_data_on_readers operation in the
     * SubscriberListener. That way the SubscriberListener can delegate to the
     * DataReaderListener objects the handling of the data.
     *
     * @return RETCODE_OK
     */
    ...
    /**
     * @brief Deletes all contained DataReaders. If the DataReaders have any
     * QueryCondition or ReadCondition, they are deleted before the DataReader itself.
     *
    ...
    /**
     * This operation sets a default value of the DataReader QoS policies which will be
     * used for newly created DataReader entities in the case where the QoS policies are
     * defaulted in the create_datareader operation.
     *
     ...
     *
     * The special value DATAREADER_QOS_DEFAULT may be passed to this operation to
    ...
    /**
     * This operation returns the default value of the DataReader QoS, that is, the QoS
     * policies which will be used for newly created DataReader entities in the case where
     * the QoS policies are defaulted in the create_datareader operation.
     *
     * The values retrieved get_default_datareader_qos will match the set of values
     * specified on the last successful call to get_default_datareader_qos, or else, if
    ...
    /**
     * This operation retrieves the default value of the DataReader QoS, that is, the QoS
     * policies which will be used for newly created DataReader entities in the case where
     * the QoS policies are defaulted in the create_datareader operation.
     *
     * The values retrieved get_default_datareader_qos will match the set of values
     ...
     * @return RETCODE_OK if successful, an error code otherwise
     */
    RTPS_DllAPI static ReturnCode_t copy_from_topic_qos(DataReaderQos &reader_qos,
