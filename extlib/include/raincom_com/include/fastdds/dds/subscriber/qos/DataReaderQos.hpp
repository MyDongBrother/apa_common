...
class ReaderResourceLimitsQos
{
  public:
     ...
     *
     * This attribute is a signed integer to be consistent with the @c max_samples
     * argument of
     * @ref DataReader methods, but should always have a strict positive value. Bear in
     * mind that a big number here may cause the creation of the DataReader to fail due to
     * pre-allocation of internal resources.
     *
     * Default value: 32.
     */
  ...
  public:
    ...
    bool operator==(const TypeConsistencyQos &b) const
    {
        return (this->type_consistency == b.type_consistency) &&
               (this->representation == b.representation) && QosPolicy::operator==(b);
    }
...
/**
 * Class DataReaderQos, containing all the possible Qos that can be set for a determined
 * DataReader. Although these values can be set and are transmitted during the Endpoint
 * Discovery Protocol, not all of the behaviour associated with them has been implemented
 * in the library. Please consult each of them to check for implementation details and
 * default values.
  ...
  public:
    ...
    RTPS_DllAPI bool operator==(const DataReaderQos &b) const
    {
        return (durability_ == b.durability()) && (deadline_ == b.deadline()) &&
               ...
               (liveliness_ == b.liveliness()) && (reliability_ == b.reliability()) &&
               (destination_order_ == b.destination_order()) &&
               (history_ == b.history()) && (resource_limits_ == b.resource_limits()) &&
     ...
     */
    RTPS_DllAPI const DurabilityQosPolicy &durability() const { return durability_; }
     ...
     */
    RTPS_DllAPI LatencyBudgetQosPolicy &latency_budget() { return latency_budget_; }
     ...
     */
    RTPS_DllAPI const ReliabilityQosPolicy &reliability() const { return reliability_; }
     ...
     */
    RTPS_DllAPI void destination_order(const DestinationOrderQosPolicy &new_value)
    {
     ...
     */
    RTPS_DllAPI const UserDataQosPolicy &user_data() const { return user_data_; }
     ...
     */
    RTPS_DllAPI void time_based_filter(const TimeBasedFilterQosPolicy &new_value)
    {
     ...
     */
    RTPS_DllAPI void reader_data_lifecycle(const ReaderDataLifecycleQosPolicy &new_value)
    {
     ...
     */
    RTPS_DllAPI void durability_service(const DurabilityServiceQosPolicy &new_value)
    {
     ...
     */
    RTPS_DllAPI void reliable_reader_qos(const RTPSReliableReaderQos &new_value)
    {
     ...
     */
    RTPS_DllAPI TypeConsistencyQos &type_consistency() { return type_consistency_; }
     ...
     */
    RTPS_DllAPI const PropertyPolicyQos &properties() const { return properties_; }
     ...
     */
    RTPS_DllAPI void endpoint(const RTPSEndpointQos &new_value) { endpoint_ = new_value; }
     ...
     * @return DataSharingQosPolicy reference
     */
    RTPS_DllAPI const DataSharingQosPolicy &data_sharing() const { return data_sharing_; }
