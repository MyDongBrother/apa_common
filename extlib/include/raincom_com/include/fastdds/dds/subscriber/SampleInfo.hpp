...
/*!
 * @brief SampleInfo is the information that accompanies each sample that is ‘read’ or
 * ‘taken.’
 */
struct SampleInfo
{
    ...
    //! indicates whether the DataReader has already seen samples for the most-current
    //! generation of the related instance.
    ViewStateKind view_state;

    //! indicates whether the instance is currently in existence or, if it has been
    //! disposed, the reason why it was disposed.
    InstanceStateKind instance_state;

    //! number of times the instance had become alive after it was disposed
    int32_t disposed_generation_count;

    //! number of times the instance had become alive after it was disposed because no
    //! writers
    ...
    //! the generation difference between the time the sample was received, and the time
    //! the most recent sample in the collection was received.
    int32_t generation_rank;

    //! the generation difference between the time the sample was received, and the time
    //! the most recent sample was received. The most recent sample used for the
    //! calculation may or may not be in the returned collection
    ...
    //!
    //! Is the same InstanceHandle_t that is returned by the operation
    //! get_matched_publications on the DataReader
    InstanceHandle_t publication_handle;

    //! whether the DataSample contains data or is only used to communicate of a change in
    //! the instance
    bool valid_data;
