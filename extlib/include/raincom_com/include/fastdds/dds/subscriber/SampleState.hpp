 ...
 *
 * For each sample received, the middleware internally maintains a sample state relative
 * to each @ref DataReader. This sample state can have the following values:
 *
 * @li @ref READ_SAMPLE_STATE indicates that the \ref DataReader has already accessed that
 * sample by means of a read or take operation
 *
 * @li @ref NOT_READ_SAMPLE_STATE indicates that the \ref DataReader has not accessed that
 * sample before.
 *
 * The sample state will, in general, be different for each sample in the collection
