 ...
 * It would also have an inmutable @c maximum(), so it would not allow @c length() to grow
 * beyond the maximum value indicated on construction.
 */
struct UserAllocatedSequence : public LoanableCollection
{
     ...
     *
     * @param [in] items      Pointer to the beginning of an array of @c num_items opaque
     * pointers.
