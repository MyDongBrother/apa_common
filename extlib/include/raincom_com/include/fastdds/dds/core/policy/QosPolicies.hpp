...
{
  public:
    //! Boolean that indicates if the Qos has been changed with respect to the default
    //! Qos.
    bool hasChanged;
     ...
     */
    QosPolicy() : hasChanged(false), send_always_(false) {}

    /**
     * @brief Constructor
     *
     * @param send_always Boolean that set if the Qos need to be sent even if it is not
     * changed
     */
    explicit QosPolicy(bool send_always) : hasChanged(false), send_always_(send_always) {}
    ...
    {
        // hasChanged field isn't needed to be compared to being equal two QosPolicy
        // objects.
     ...
     */
    virtual bool send_always() const { return send_always_; }
...
/**
 * @brief Controls the behavior of the entity when acting as a factory for other entities.
...
class EntityFactoryQosPolicy
{
  public:
     ...
     * instances it creates. If True the factory will automatically enable each created
     * Entity otherwise it will not. <br> By default, True.
     */
    ...
    {
        return (this->autoenable_created_entities == b.autoenable_created_entities);
    }
...
typedef enum DurabilityQosPolicyKind : fastrtps::rtps::octet
{
     ...
     * DataReader that is not known by the DataWriter at the time the instance is written.
     * In other words the Service will only attempt to provide the data to existing
     * subscribers
     */
    VOLATILE_DURABILITY_QOS,
    /**
     * For TRANSIENT_LOCAL, the service is only required to keep the data in the memory of
     * the DataWriter that wrote the data and the data is not required to survive the
     * DataWriter.
     */
    TRANSIENT_LOCAL_DURABILITY_QOS,
    /**
     * For TRANSIENT, the service is only required to keep the data in memory and not in
     * permanent storage; but the data is not tied to the lifecycle of the DataWriter and
...
class DurabilityQosPolicy : public Parameter_t, public QosPolicy
{
  ...
  public:
     ...
     * By default the value for DataReaders: VOLATILE_DURABILITY_QOS, for DataWriters
     * TRANSIENT_LOCAL_DURABILITY_QOS
     */
...
/**
 * @brief DataReader expects a new sample updating the value of each instance at least
 * once every deadline period. DataWriter indicates that the application commits to write
 * a new value (using the DataWriter) for each instance managed by the DataWriter at least
...
/**
 * Specifies the maximum acceptable delay from the time the data is written until the data
 * is inserted in the receiver's application-cache and the receiving application is
 * notified of the fact.This policy is a hint to the Service, not something that must be
 * monitored or enforced. The Service is not required to track or alert the user of any
 * violation.
 *
 * @warning This QosPolicy can be defined and is transmitted to the rest of the network
...
class LatencyBudgetQosPolicy : public Parameter_t, public QosPolicy
{
  ...
  public:
    //! Maximum acceptable delay from the time data is written until it is received. <br>
...
{
    /**
     * The infrastructure will automatically signal liveliness for the DataWriters at
     * least as often as required by the lease_duration.
     */
    AUTOMATIC_LIVELINESS_QOS,
    /**
     * The Service will assume that as long as at least one Entity within the
     * DomainParticipant has asserted its liveliness the other Entities in that same
...
/**
 * Determines the mechanism and parameters used by the application to determine whether an
 * Entity is “active” (alive). The “liveliness” status of an Entity is used to maintain
 * instance ownership in combination with the setting of the OwnershipQosPolicy. The
 * application is also informed via listener when an Entity is no longer alive.
 *
 * The DataReader requests that liveliness of the writers is maintained by the requested
 * means and loss of liveliness is detected with delay not to exceed the lease_duration.
 *
 * The DataWriter commits to signaling its liveliness using the stated means at intervals
 * not to exceed the lease_duration. Listeners are used to notify the DataReaderof loss of
 * liveliness and DataWriter of violations to the liveliness contract.
 */
class LivelinessQosPolicy : public Parameter_t, public QosPolicy
{
  ...
  public:
     ...
     *  On a DataReader it represents the period without assertion after which a
     * DataWriter is considered inactive. By default, c_TimeInfinite.
     */
     ...
     *
     * @warning When not infinite, must be < lease_duration, and it is advisable to be
     * less than 0.7*lease_duration.
     */
...
{
    /**
     * Indicates that it is acceptable to not retry propagation of any samples. Presumably
     * new values for the samples are generated often enough that it is not necessary to
     * re-send or acknowledge any samples
     */
    BEST_EFFORT_RELIABILITY_QOS = 0x01,
    /**
     * Specifies the Service will attempt to deliver all samples in its history. Missed
     * samples may be retried. In steady-state (no modifications communicated via the
     * DataWriter) the middleware guarantees that all samples in the DataWriter history
     * will eventually be delivered to all the DataReader objects. Outside steady state
     * the HistoryQosPolicy and ResourceLimitsQosPolicy will determine how samples become
...
class ReliabilityQosPolicy : public Parameter_t, public QosPolicy
{
  ...
  public:
     ...
     * By default, BEST_EFFORT_RELIABILITY_QOS for DataReaders and
...
enum OwnershipQosPolicyKind : fastrtps::rtps::octet
{
     ...
     * update the same instance and all the updates are made available to the readers. In
     * other words there is no concept of an “owner” for the instances.
     */
    SHARED_OWNERSHIP_QOS,
    /**
     * Indicates each instance can only be owned by one DataWriter, but the owner of an
     * instance can change dynamically. The selection of the owner is controlled by the
     * setting of the OwnershipStrengthQosPolicy. The owner is always set to be the
     * highest-strength DataWriter object among the ones currently “active” (as determined
     * by the LivelinessQosPolicy).
     */
    EXCLUSIVE_OWNERSHIP_QOS
};

/**
 * Specifies whether it is allowed for multiple DataWriters to write the same instance of
 * the data and if so, how these modifications should be arbitrated
 *
 * @note Immutable Qos Policy
 */
...
/**
 * Enum DestinationOrderQosPolicyKind, different kinds of destination order for
...
enum DestinationOrderQosPolicyKind : fastrtps::rtps::octet
{
     ...
     * guaranteed that the changes will be seen in the same order. Consequently, it is
     * possible for each subscriber to end up with a different final value for the data.
     */
    BY_RECEPTION_TIMESTAMP_DESTINATIONORDER_QOS,
    /**
     * Indicates that data is ordered based on a timestamp placed at the source (by the
     * Service or by the application). In any case this guarantees a consistent final
     * value for the data in all subscribers.
     */
 ...
 * Publisher entities to the same instance of data (i.e., matching Topic and key).
 *
 * @warning This QosPolicy can be defined and is transmitted to the rest of the network
 * but is not implemented in this version.
 *
...
class DestinationOrderQosPolicy : public Parameter_t, public QosPolicy
{
  ...
  public:
    //! DestinationOrderQosPolicyKind. <br> By default,
    //! BY_RECEPTION_TIMESTAMP_DESTINATIONORDER_QOS.
    DestinationOrderQosPolicyKind kind;
};

/**
 * Class GenericDataQosPolicy, base class to transmit user data during the discovery
 * phase.
 */
class GenericDataQosPolicy : public Parameter_t,
                             public QosPolicy,
                             public fastrtps::ResourceLimitedVector<fastrtps::rtps::octet>
{
  ...
  public:
    ...
    RTPS_DllAPI GenericDataQosPolicy(ParameterId_t pid, uint16_t in_length)
        : Parameter_t(pid, in_length), QosPolicy(false), ResourceLimitedOctetVector()
    {
    }
     ...
     * @param data data to copy in the newly created object
     */
    RTPS_DllAPI GenericDataQosPolicy(ParameterId_t pid, const collection_type &data)
        : Parameter_t(pid, 0), QosPolicy(false), ResourceLimitedOctetVector()
    {
    ...
    {
        return collection_ == b.collection_ && Parameter_t::operator==(b) &&
               QosPolicy::operator==(b);
    }

    bool operator==(const collection_type &b) const { return collection_ == b; }
    ...
    void set_max_size(size_t size)
    {
        ...
        {
            configuration_ =
                fastrtps::ResourceLimitedContainerConfig::fixed_size_configuration(size);
    ...
    void resize(size_t new_size) { collection_.resize(new_size); }

    /**
     * @return const reference to the internal raw data.
     */
    inline const collection_type &dataVec() const { return collection_; }
     ...
     * */
    RTPS_DllAPI inline const collection_type &data_vec() const { return collection_; }
     ...
     * @return raw data as vector of octets.
     * */
    RTPS_DllAPI inline const collection_type &getValue() const { return collection_; }
     ...
     * @param vec raw data to set.
     * */
    RTPS_DllAPI inline void setValue(const collection_type &vec) { data_vec(vec); }
};
...
#define TEMPLATE_DATA_QOS_POLICY(TClassName, TPid)                                  \
    ...
    {                                                                               \
      public:                                                                       \
         ...
         * used. The resulting TClassName will always be unlimited in size                          \
         *                                                                          \
         * @param data data to copy in the newly created object                     \
         */                                                                         \
 ...
 *
 * The purpose of this QoS is to allow the application to attach additional information to
 * the created Entity objects such that when a remote application discovers their
 ...
 * information that can be used by the remote application to authenticate the source.
 */
 ...
 *
 * The purpose of this QoS is to allow the application to attach additional information to
 * the created Topic such that when a remote application discovers their existence it can
 * examine the information and use it in an application-defined way.
 *
 * In combination with the listeners on the DataReader and DataWriter as well as by means
 * of operations such as ignore_topic,these QoS can assist an application to extend the
 * provided QoS.
 */
 ...
 *
 * The purpose of this QoS is to allow the application to attach additional information to
 * the created Publisher or Subscriber. The value of the GROUP_DATA is available to the
 * application on the DataReader and DataWriter entities and is propagated by means of the
 * built-in topics.
 *
 * This QoS can be used by an application combination with the DataReaderListener and
 * DataWriterListener to implement matching policies similar to those of the PARTITION QoS
...
/**
 * Filter that allows a DataReader to specify that it is interested only in (potentially)
 * a subset of the values of the data. The filter states that the DataReader does not want
 * to receive more than one value each minimum_separation, regardless of how fast the
 * changes occur. It is inconsistent for a DataReader to have a minimum_separation longer
...
class TimeBasedFilterQosPolicy : public Parameter_t, public QosPolicy
{
  ...
  public:
    //! Minimum interval between samples. By default, c_TimeZero (the DataReader is
    //! interested in all values)
    fastrtps::Duration_t minimum_separation;
};

/**
 * Enum PresentationQosPolicyAccessScopeKind, different kinds of Presentation Policy order
 * for PresentationQosPolicy.
 */
enum PresentationQosPolicyAccessScopeKind : fastrtps::rtps::octet
{
    /**
     * Scope spans only a single instance. Indicates that changes to one instance need not
     * be coherent nor ordered with respect to changes to any other instance. In other
     * words, order and coherent changes apply to each instance separately.
     */
    INSTANCE_PRESENTATION_QOS,
    /**
     * Scope spans to all instances within the same DataWriter (or DataReader), but not
     * across instances in different DataWriter (or DataReader).
     */
    TOPIC_PRESENTATION_QOS,
    /**
     * Scope spans to all instances belonging to DataWriter (or DataReader) entities
...
/**
 * Specifies how the samples representing changes to data instances are presented to the
 * subscribing application. This policy affects the application’s ability to specify and
 * receive coherent changes and to see the relative order of changes.access_scope
 * determines the largest scope spanning the entities for which the order and coherency of
 * changes can be preserved. The two booleans control whether coherent access and ordered
 * access are supported within the scope access_scope.
 *
 * @warning This QosPolicy can be defined and is transmitted to the rest of the network
 * but is not implemented in this version.
 *
...
class PresentationQosPolicy : public Parameter_t, public QosPolicy
{
  public:
    ...
    bool operator==(const PresentationQosPolicy &b) const
    {
        return (this->access_scope == b.access_scope) &&
               ...
               (this->ordered_access == b.ordered_access) && Parameter_t::operator==(b) &&
               QosPolicy::operator==(b);
    }
  ...
  public:
    ...
    /**
     * @brief Specifies support coherent access. That is, the ability to group a set of
     * changes as a unit on the publishing end such that they are received as a unit at
     * the subscribing end. by default, false.
     */
    bool coherent_access;
    /**
     * @brief Specifies support for ordered access to the samples received at the
     * subscription end. That is, the ability of the subscriber to see changes in the same
     * order as they occurred on the publishing end. By default, false.
     */
    bool ordered_access;
};
...
/**
 * Set of strings that introduces a logical partition among the topics visible by the
 * Publisher and Subscriber. A DataWriter within a Publisher only communicates with a
 * DataReader in a Subscriber if (in addition to matching the Topic and having compatible
 * QoS) the Publisher and Subscriber have a common partition name string.
 *
 * The empty string ("") is considered a valid partition that is matched with other
 * partition names using the same rules of string matching and regular-expression matching
 * used for any other partition name.
 *
 * @note Mutable Qos Policy
 */
class PartitionQosPolicy : public Parameter_t, public QosPolicy
{
  public:
    class const_iterator
    {
      public:
         ...
         * @param ptr Pointer to be set
         */
        const_iterator(const fastrtps::rtps::octet *ptr) : ptr_(ptr), value_(ptr_) {}
  ...
  public:
    ...
    RTPS_DllAPI PartitionQosPolicy()
        : Parameter_t(PID_PARTITION, 0), QosPolicy(false), max_size_(0), Npartitions_(0)
    {
    }
    ...
    RTPS_DllAPI PartitionQosPolicy(const PartitionQosPolicy &b)
        : Parameter_t(b),
          ...
          partitions_(b.max_size_ != 0 ? b.partitions_.max_size : b.partitions_.length),
    ...
    bool operator==(const PartitionQosPolicy &b) const
    {
        return (this->max_size_ == b.max_size_) &&
               ...
               (this->partitions_ == b.partitions_) && Parameter_t::operator==(b) &&
               QosPolicy::operator==(b);
    }
    ...
    PartitionQosPolicy &operator=(const PartitionQosPolicy &b)
    {
        ...
        Parameter_t::operator=(b);
        max_size_ = b.max_size_;
        partitions_.reserve(max_size_ != 0 ? b.partitions_.max_size
     ...
     */
    uint32_t empty() const { return Npartitions_ == 0; }
...
enum HistoryQosPolicyKind : fastrtps::rtps::octet
{
     ...
     * “depth” samples of each instance of data (identified by its key) managed by the
     * DataWriter. On the subscribing side, the DataReader will only attempt to keep the
     * most recent “depth” samples received for each instance (identified by its key)
     * until the application “takes” them via the DataReader’s take operation.
     */
    KEEP_LAST_HISTORY_QOS,
    /**
     * On the publishing side, the Service will attempt to keep all samples (representing
     * each value written) of each instance of data (identified by its key) managed by the
     * DataWriter until they can be delivered to all subscribers. On the subscribing side,
     * the Service will attempt to keep all samples of each instance of data (identified
     * by its key) managed by the DataReader. These samples are kept until the application
     * “takes” them from the Service via the take operation.
     */
 ...
 * (one or more times) before it can be successfully communicated to one or more existing
 * subscribers. This QoS policy controls whether the Service should deliver only the most
 * recent value, attempt to deliver all intermediate values, or do something in between.
 * On the publishing side this policy controls the samples that should be maintained by
 * the DataWriter on behalf of existing DataReader entities. The behavior with regards to
 * a DataReaderentities discovered after a sample is written is controlled by the
 * DURABILITY QoS policy. On the subscribing side it controls the samples that should be
 * maintained until the application “takes” them from the Service.
 *
 * @note Immutable Qos Policy
 */
...
class ResourceLimitsQosPolicy : public Parameter_t, public QosPolicy
{
  public:
    /**
     * @brief Specifies the maximum number of data-samples the DataWriter (or DataReader)
     * can manage across all the instances associated with it. Represents the maximum
     * samples the middleware can store for any one DataWriter (or DataReader). <br> By
     * default, 5000.
     *
    ...
    int32_t max_samples;
    /**
     * @brief Represents the maximum number of instances DataWriter (or DataReader) can
     * manage. <br> By default, 10.
     */
    ...
    /**
     * @brief Represents the extra number of samples available once the max_samples have
     * been reached in the history. This makes it possible, for example, to loan samples
     * even with a full history. By default, 1.
     */
...
/**
 * Specifies the configuration of the durability service. That is, the service that
 * implements the DurabilityQosPolicy kind of TRANSIENT and PERSISTENT.
 *
 * @warning This QosPolicy can be defined and is transmitted to the rest of the network
 * but is not implemented in this version.
 *
...
class DurabilityServiceQosPolicy : public Parameter_t, public QosPolicy
{
  public:
    ...
    RTPS_DllAPI DurabilityServiceQosPolicy()
        : Parameter_t(PID_DURABILITY_SERVICE,
                      PARAMETER_TIME_LENGTH + PARAMETER_KIND_LENGTH + 4 + 4 + 4 + 4),
          QosPolicy(false),
  ...
  public:
    /**
     * @brief Control when the service is able to remove all information regarding a
     * data-instance. <br> By default, c_TimeZero.
     */
    fastrtps::Duration_t service_cleanup_delay;
    /**
     * @brief Controls the HistoryQosPolicy of the fictitious DataReader that stores the
     * data within the durability service. <br> By default, KEEP_LAST_HISTORY_QOS.
     */
    HistoryQosPolicyKind history_kind;
    /**
     * @brief Number of most recent values that should be maintained on the History. It
     * only have effect if the history_kind is KEEP_LAST_HISTORY_QOS. <br> By default, 1.
     */
    int32_t history_depth;
    /**
     * @brief Control the ResourceLimitsQos of the implied DataReader that stores the data
     * within the durability service. Specifies the maximum number of data-samples the
     * DataWriter (or DataReader) can manage across all the instances associated with it.
     * Represents the maximum samples the middleware can store for any one DataWriter (or
     * DataReader). It is inconsistent for this value to be less than
     * max_samples_per_instance. <br> By default, LENGTH_UNLIMITED.
     */
    int32_t max_samples;
    /**
     * @brief Control the ResourceLimitsQos of the implied DataReader that stores the data
     * within the durability service. Represents the maximum number of instances
     * DataWriter (or DataReader) can manage. <br> By default, LENGTH_UNLIMITED.
     */
    int32_t max_instances;
    /**
     * @brief Control the ResourceLimitsQos of the implied DataReader that stores the data
     * within the durability service. Represents the maximum number of samples of any one
     * instance a DataWriter(or DataReader) can manage. It is inconsistent for this value
     * to be greater than max_samples. <br> By default, LENGTH_UNLIMITED.
     */
    int32_t max_samples_per_instance;
};
...
/**
 * Specifies the value of the “strength” used to arbitrate among multiple DataWriter
 * objects that attempt to modify the same instance of a data-object (identified by Topic
...
class PublishModeQosPolicy : public QosPolicy
{
  public:
    ...
    /*! Name of the flow controller used when publish mode kind is
...
/**
 * With multiple standard data Representations available, and vendor-specific extensions
 * possible, DataWriters and DataReaders must be able to negotiate which data
 * representation(s) to use. This negotiation shall occur based on
 * DataRepresentationQosPolicy.
 *
 * @warning If a writer’s offered representation is contained within a reader’s sequence,
 * the offer satisfies the request and the policies are compatible. Otherwise, they are
 * incompatible.
 *
 * @note Immutable Qos Policy
 */
...
{
    /**
     * The DataWriter and the DataReader must support the same data type in order for them
     * to communicate.
     */
    DISALLOW_TYPE_COERCION,
    /**
     * The DataWriter and the DataReader need not support the same data type in order for
     * them to communicate as long as the reader’s type is assignable from the writer’s
     * type.
     */
    ALLOW_TYPE_COERCION
};
...
/**
 * The TypeConsistencyEnforcementQosPolicy defines the rules for determining whether the
 * type used to publish a given data stream is consistent with that used to subscribe to
 * it. It applies to DataReaders.
 *
 * @note Immutable Qos Policy
 */
class TypeConsistencyEnforcementQosPolicy : public Parameter_t, public QosPolicy
{
  public:
    ...
    /**
     * @brief This option controls whether sequence bounds are taken into consideration
     * for type assignability. If the option is set to TRUE, sequence bounds (maximum
     * lengths) are not considered as part of the type assignability. This means that a T2
     * sequence type with maximum length L2 would be assignable to a T1 sequence type with
     * maximum length L1, even if L2 is greater than L1. If the option is set to false,
     * then sequence bounds are taken into consideration for type assignability and in
     * order for T1 to be assignable from T2 it is required that L1>= L2. <br> By default,
     * true.
     */
    bool m_ignore_sequence_bounds;
    /**
     * @brief This option controls whether string bounds are taken into consideration for
     * type assignability. If the option is set to TRUE, string bounds (maximum lengths)
     * are not considered as part of the type assignability. This means that a T2 string
     * type with maximum length L2 would be assignable to a T1 string type with maximum
     * length L1, even if L2 is greater than L1. If the option is set to false, then
     * string bounds are taken into consideration for type assignability and in order for
     * T1 to be assignable from T2 it is required that L1>= L2. <br> By default, true.
     */
    bool m_ignore_string_bounds;
    /**
     * @brief This option controls whether member names are taken into consideration for
     * type assignability. If the option is set to TRUE, member names are considered as
     * part of assignability in addition to member IDs (so that members with the same ID
     * also have the same name). If the option is set to FALSE, then member names are not
     * ignored. <br> By default, false.
     */
    bool m_ignore_member_names;
    /**
     * @brief This option controls whether type widening is allowed. If the option is set
     * to FALSE, type widening is permitted. If the option is set to TRUE,it shall cause a
     * wider type to not be assignable to a narrower type. <br> By default, false.
     */
    bool m_prevent_type_widening;
    /**
     * @brief This option requires type information to be available in order to complete
     * matching between a DataWriter and DataReader when set to TRUE, otherwise matching
     * can occur without complete type information when set to FALSE. <br> By default,
     * false.
     */
...
class DisablePositiveACKsQosPolicy : public Parameter_t, public QosPolicy
{
  public:
    ...
    {
        return enabled == b.enabled && Parameter_t::operator==(b) &&
               QosPolicy::operator==(b);
    }
  ...
  public:
    ...
    bool enabled;
    //! The duration to keep samples for (not serialized as not needed by reader). <br> By
    //! default, c_TimeInfinite
    fastrtps::Duration_t duration;
};
...
class TypeIdV1 : public Parameter_t, public QosPolicy
{
  public:
    ...
    RTPS_DllAPI TypeIdV1(const fastrtps::types::TypeIdentifier &identifier)
        : Parameter_t(PID_TYPE_IDV1, 0), QosPolicy(false), m_type_identifier(identifier)
    {
    }
    ...
    inline void clear() override { *this = TypeIdV1(); }

    RTPS_DllAPI TypeIdV1 &operator=(const fastrtps::types::TypeIdentifier &type_id)
    {
...
class TypeObjectV1 : public Parameter_t, public QosPolicy
{
  public:
    ...
    RTPS_DllAPI TypeObjectV1(const fastrtps::types::TypeObject &type)
        : Parameter_t(PID_TYPE_OBJECTV1, 0), QosPolicy(false), m_type_object(type)
    {
    }
    ...
    inline void clear() override { *this = TypeObjectV1(); }

    RTPS_DllAPI TypeObjectV1 &operator=(const fastrtps::types::TypeObject &type_object)
    {
     ...
     */
    RTPS_DllAPI const fastrtps::types::TypeObject &get() const { return m_type_object; }
};
...
class WireProtocolConfigQos : public QosPolicy
{
  ...
  public:
     ...
     * RTPSParticipant in the case that it was defined with NO UnicastLocators. At least
     * ONE locator should be included in this list.
     */
     ...
     * RTPSParticipant in the case that it was defined with NO UnicastLocators. This is
     * usually left empty.
     */
...
class TransportConfigQos : public QosPolicy
{
  public:
    ...
    /*!
     * @brief Send socket buffer size for the send resource. Zero value indicates to use
     * default system buffer size. <br> By default, 0.
     */
    uint32_t send_socket_buffer_size;

    /*! Listen socket buffer for all listen resources. Zero value indicates to use default
...
class RTPSEndpointQos
{
  public:
    ...
    //! Entity ID, if the user wants to specify the EntityID of the endpoint. <br> By
    //! default, -1.
    int16_t entity_id;
...
class DataSharingQosPolicy : public Parameter_t, public QosPolicy
{
  public:
     ...
     * @brief Constructor
     */
    RTPS_DllAPI DataSharingQosPolicy() : Parameter_t(PID_DATASHARING, 0), QosPolicy(true)
    {
    ...
    RTPS_DllAPI DataSharingQosPolicy(const DataSharingQosPolicy &b)
        : Parameter_t(b),
          ...
          domain_ids_(b.max_domains() != 0 ? b.max_domains() : b.domain_ids().size())
    {
    ...
    RTPS_DllAPI DataSharingQosPolicy &operator=(const DataSharingQosPolicy &b)
    {
        ...
        shm_directory_ = b.shm_directory();
        max_domains_   = b.max_domains();
        domain_ids_.reserve(max_domains_ != 0 ? max_domains_ : b.domain_ids().size());
        domain_ids_ = b.domain_ids();
    ...
    {
        return kind_ == b.kind_ && shm_directory_ == b.shm_directory_ &&
               domain_ids_ == b.domain_ids_ && Parameter_t::operator==(b) &&
               QosPolicy::operator==(b);
    }
     ...
     */
    RTPS_DllAPI const std::string &shm_directory() const { return shm_directory_; }
     ...
     */
    RTPS_DllAPI const std::vector<uint64_t> &domain_ids() const { return domain_ids_; }
     ...
     *
     * @param directory The shared memory directory to use.
     *      It is mandatory to provide a non-empty name or the creation of endpoints will
     * fail.
     */
    RTPS_DllAPI void on(const std::string &directory)
    {
        // TODO [ILG]: This parameter is unused right now. Activate the assert once it is
        // used
