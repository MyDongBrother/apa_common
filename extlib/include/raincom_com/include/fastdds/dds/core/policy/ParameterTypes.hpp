...
class ParameterLocator_t : public Parameter_t
{
  public:
    ...
    ParameterLocator_t(ParameterId_t pid, uint16_t in_length, const rtps::Locator &loc)
        : Parameter_t(pid, in_length), locator(loc)
    {
    }
};
...
class ParameterString_t : public Parameter_t
{
  public:
     ...
     */
    inline const char *getName() const { return string_.c_str(); }
     ...
     */
    inline void setName(const char *name) { string_ = name; }
...
class ParameterPort_t : public Parameter_t
{
  public:
    ...
    ParameterPort_t(ParameterId_t pid, uint16_t in_length)
        : Parameter_t(pid, in_length), port(0)
    {
    }
    ...
    ParameterPort_t(ParameterId_t pid, uint16_t in_length, uint32_t po)
        : Parameter_t(pid, in_length), port(po)
    {
    }
};
...
class ParameterGuid_t : public Parameter_t
{
  public:
    ...
    /**
     * @brief Constructor using a parameter PID, the parameter length and a Instance
     * Handle
     *
     * @param pid Pid of the parameter
     * @param in_length Its associated length
...
class ParameterVendorId_t : public Parameter_t
{
  public:
    ...
    ParameterVendorId_t(ParameterId_t pid, uint16_t in_length)
        : Parameter_t(pid, in_length), vendorId(fastrtps::rtps::c_VendorId_eProsima)
    {
    }
};
...
class ParameterBool_t : public Parameter_t
{
  public:
    ...
    ParameterBool_t(ParameterId_t pid, uint16_t in_length)
        : Parameter_t(pid, in_length), value(false)
    {
    }
    ...
    ParameterBool_t(ParameterId_t pid, uint16_t in_length, bool inbool)
        : Parameter_t(pid, in_length), value(inbool)
    {
    }
};
...
class ParameterStatusInfo_t : public Parameter_t
{
  public:
    ...
    ParameterStatusInfo_t(ParameterId_t pid, uint16_t in_length)
        : Parameter_t(pid, in_length), status(0)
    {
    }
    ...
    ParameterStatusInfo_t(ParameterId_t pid, uint16_t in_length, uint8_t instatus)
        : Parameter_t(pid, in_length), status(instatus)
    {
    }
};
...
class ParameterProperty_t
{
  ...
  public:
     ...
     * @brief Constructor without parameters
     */
    ParameterProperty_t() { data = nullptr; }
     ...
     */
    explicit ParameterProperty_t(void *ptr) { data = (fastrtps::rtps::octet *)ptr; }
     ...
     * @return true if the modification is done correctly and false if the size of the
    ...
    bool modify(const std::pair<std::string, std::string> &new_value)
    {
        ...
        uint32_t new_size =
            first_size + first_alignment + second_size + second_alignment + 8;
    ...
    bool operator!=(const ParameterProperty_t &b) const { return !(*this == b); }
...
class ParameterPropertyList_t : public Parameter_t
{
  ...
  public:
    ...
    ParameterPropertyList_t()
        : Parameter_t(PID_PROPERTY_LIST, 0), Nproperties_(0), limit_size_(false)
    {
    }
    ...
    ParameterPropertyList_t(ParameterId_t pid, uint16_t in_length)
        : Parameter_t(PID_PROPERTY_LIST, in_length), Nproperties_(0), limit_size_(false)
    {
    ...
    {
        properties_.copy(&parameter_properties.properties_,
                         parameter_properties.limit_size_);
    }
        ...
        const ParameterPropertyList_t &parameter_properties)
    {
        ...
        limit_size_ = parameter_properties.limit_size_;
        properties_.reserve(limit_size_ ? parameter_properties.properties_.max_size
                                        : parameter_properties.properties_.length);
        properties_.copy(&parameter_properties.properties_,
                         parameter_properties.limit_size_);
        Nproperties_ = parameter_properties.Nproperties_;
     ...
     * @return iterator
     */
    iterator begin() { return iterator(properties_.data); }
     ...
     * @return iterator
     */
    iterator end() { return iterator(properties_.data + properties_.length); }
     ...
     */
    const_iterator begin() const { return const_iterator(properties_.data); }
     ...
     * @return true if it is introduced, false if not.
     */
    bool push_back(const unsigned char *str1, uint32_t str1_size,
                   const unsigned char *str2, uint32_t str2_size)
    {
        // Realloc if needed;
        ...
        if (limit_size_ &&
            (properties_.max_size < properties_.length + str1_size + alignment1 + 4 +
                                        str2_size + alignment2 + 4))
        {
            return false;
        }
        properties_.reserve(properties_.length + str1_size + alignment1 + 4 + str2_size +
                            alignment2 + 4);
     ...
     * @return uint32_t with the size
     */
    uint32_t size() const { return Nproperties_; }
     ...
     * @return uint32_t with the size
     */
    uint32_t max_size() { return (limit_size_ ? properties_.max_size : 0); }
...
class ParameterSampleIdentity_t : public Parameter_t
{
  public:
     ...
     * @brief Constructor without parameters
     */
    ParameterSampleIdentity_t() : sample_id(fastrtps::rtps::SampleIdentity::unknown()) {}
