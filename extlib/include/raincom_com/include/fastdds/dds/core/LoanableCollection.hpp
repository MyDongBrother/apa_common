...
/**
 * A collection of generic opaque pointers that can receive the buffer from outside
 * (loan).
 *
...
class LoanableCollection
{
  public:
     ...
     */
    const element_type *buffer() const { return elements_; }
     ...
     */
    size_type maximum() const { return maximum_; }
     ...
     *
     * This method tells the collection that a certain number of elements should be
     * accessible. If the new length is greater than the current @ref maximum() the
     * collection should allocate space for the new elements. If this is the case and the
     * collection does not own the buffer (i.e. @ref has_ownership() is false) then no
     * allocation will be performed, the length will remain unchanged, and false will be
     * returned.
     *
     * @pre new_length >= 0
     *
