...
{
    //! @brief Constructor
    DeadlineMissedStatus() : total_count(), total_count_change(), last_instance_handle()
    {
    }
    ...
    //! @brief Total cumulative number of offered deadline periods elapsed during which a
    //! writer failed to provide data
    //! @details Missed deadlines accumulate, that is, each deadline period the
