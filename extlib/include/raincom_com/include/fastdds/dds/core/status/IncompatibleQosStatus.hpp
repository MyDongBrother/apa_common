...
/**
 * @brief A struct storing the id of the incompatible QoS Policy and the number of times
 * it fails
 */
struct QosPolicyCount
{
    ...
    //! @brief Total number of times that the concerned writer discovered a reader for the
    //! same topic
...
{
    //! @brief Total cumulative number of times the concerned writer discovered a reader
    //! for the same topic
    //! @details The requested QoS is incompatible with the one offered by the writer
    uint32_t total_count = 0;

    //! @brief The change in total_count since the last time the listener was called or
    //! the status was read
    uint32_t total_count_change = 0;

    //! @brief The id of the policy that was found to be incompatible the last time an
    ...
    IncompatibleQosStatus()
        : policies(static_cast<size_t>(NEXT_QOS_POLICY_ID),
