...
/**
 * A type-safe accessible collection of generic opaque pointers that can receive the
 * buffer from outside (loan).
 *
...
class LoanableTypedCollection : public LoanableCollection
{
  public:
     ...
     *
     * This is the operator that is invoked when the application indexes into a @em
     * non-const sequence:
     * @code{.cpp}
     * element = sequence[n];
     * sequence[n] = element;
     ...
     *
     * This is the operator that is invoked when the application indexes into a @em const
     * sequence:
     * @code{.cpp}
     * element = sequence[n];
     * @endcode
