...
/**
 * @brief The WaitSet class allows an application to wait until one or more of the
 * attached Condition objects has a trigger_value of TRUE or until timeout expires.
 *
 */
class WaitSet
{
  public:
     ...
     * @param cond Condition
     * @return RETCODE_OK if detached correctly, PRECONDITION_NOT_MET if condition was not
     * attached
     */
     ...
     * conditions. If none of the conditions attached to the WaitSet have a trigger_value
     ...
     * @return RETCODE_OK if everything correct, PRECONDITION_NOT_MET if WaitSet already
     * waiting, TIMEOUT if maximum time expired, error code otherwise
     */
