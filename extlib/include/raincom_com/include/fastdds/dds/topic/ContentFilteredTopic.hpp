...
class ContentFilteredTopic : public TopicDescription
{
  ...
  public:
     ...
     * It will return the @c filter_expression specified on the last successful call to @c
     * set_expression or, if that method is never called, the expression specified when
     ...
     * ContentFilteredTopic. These will be the @c expression_parameters specified on the
     * last successful call to @c set_expression or
     * @c set_expression_parameters.
     * If those methods have never been called, the expression parameters specified when
     * the ContentFilteredTopic was created will be returned.
     *
     * @param [out] expression_parameters  The expression parameters currently associated
     * with the ContentFilteredTopic.
     *
     * @return RETCODE_OK
     */
    RTPS_DllAPI ReturnCode_t
    get_expression_parameters(std::vector<std::string> &expression_parameters) const;
     ...
     * updated.
     * @return RETCODE_BAD_PARAMETER  if the expression parameters do not match with the
     * current @c filter_expression.
     */
    RTPS_DllAPI ReturnCode_t
    set_expression_parameters(const std::vector<std::string> &expression_parameters);
     ...
     * @param [in] expression_parameters  The expression parameters to set.
     *
     * @return RETCODE_OK             if the expression and parameters where correctly
     * updated.
     * @return RETCODE_BAD_PARAMETER  if @c filter_expression is not valid for this
     * ContentFilteredTopic.
     * @return RETCODE_BAD_PARAMETER  if the expression parameters do not match with the
     * @c filter_expression.
     */
