 ...
 * @brief Class TypeSupport used to provide the DomainRTPSParticipant with the methods to
 * serialize, deserialize and get the key of a specific data type. The user should created
 * a class that inherits from this one, where Serialize and deserialize methods MUST be
 * implemented.
...
class TypeSupport : public std::shared_ptr<fastdds::dds::TopicDataType>
{
  public:
     ...
     * The passed pointer will be managed by the TypeSupport object, so creating two
     * TypeSupport from the same pointer or deleting the passed pointer will produce a
     * runtime error.
     *
     * @param ptr
     */
     ...
     * RETCODE_PRECONDITION_NOT_MET if there is another type with the same name registered
     * on the DomainParticipant and RETCODE_OK if it is registered correctly
     */
    RTPS_DllAPI virtual ReturnCode_t register_type(DomainParticipant *participant) const;
     ...
     */
    RTPS_DllAPI virtual bool serialize(void *data,
     ...
     */
    RTPS_DllAPI virtual bool deserialize(fastrtps::rtps::SerializedPayload_t *payload,
                                         void *data);
     ...
     * @return function
     */
    RTPS_DllAPI virtual std::function<uint32_t()> get_serialized_size_provider(void *data)
    {
     ...
     */
    RTPS_DllAPI virtual void delete_data(void *data) { return get()->deleteData(data); }
     ...
     */
    RTPS_DllAPI virtual inline bool is_bounded() const { return get()->is_bounded(); }
