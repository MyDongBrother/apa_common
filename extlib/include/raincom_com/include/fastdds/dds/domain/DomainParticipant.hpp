...
/**
 * Class DomainParticipant used to group Publishers and Subscribers into a single working
 * unit.
 *
...
class DomainParticipant : public Entity
{
  public:
     ...
     */
    RTPS_DllAPI Publisher *create_publisher(const PublisherQos &qos,
    ...
    RTPS_DllAPI Publisher *create_publisher_with_profile(
        const std::string &profile_name, PublisherListener *listener = nullptr,
        const StatusMask &mask = StatusMask::all());
     ...
     * @return RETCODE_PRECONDITION_NOT_MET if the publisher does not belong to this
     * participant or if it has active DataWriters, RETCODE_OK if it is correctly deleted
     ...
     */
    RTPS_DllAPI Subscriber *create_subscriber(const SubscriberQos &qos,
    ...
    RTPS_DllAPI Subscriber *create_subscriber_with_profile(
        const std::string &profile_name, SubscriberListener *listener = nullptr,
        const StatusMask &mask = StatusMask::all());
     ...
     * @return RETCODE_PRECONDITION_NOT_MET if the subscriber does not belong to this
     * participant or if it has active DataReaders, RETCODE_OK if it is correctly deleted
     * and RETCODE_ERROR otherwise.
     */
     ...
     * @return RETCODE_BAD_PARAMETER if the topic passed is a nullptr,
     * RETCODE_PRECONDITION_NOT_MET if the topic does not belong to this participant or if
     * it is referenced by any entity and RETCODE_OK if the Topic was deleted.
     */
     ...
     * @return nullptr if a filter cannot be created with the specified @c
     ...
     * @return RETCODE_BAD_PARAMETER if the topic passed is a nullptr,
     * RETCODE_PRECONDITION_NOT_MET if the topic does not belong to this participant or if
     * it is referenced by any entity and RETCODE_OK if the ContentFilteredTopic was
     * deleted.
     */
    RTPS_DllAPI ReturnCode_t
    delete_contentfilteredtopic(const ContentFilteredTopic *a_contentfilteredtopic);
     ...
     * @return Pointer to the topic description, if it has been created locally.
     * Otherwise, nullptr is returned.
     *
     * @remark UNSAFE. It is unsafe to lookup a topic description while another thread is
     * creating a topic.
     */
     ...
     * This is used in combination with the LIVELINESS QoS policy to indicate to the
     * Service that the entity remains active.
     *
     * This operation needs to only be used if the DomainParticipant contains DataWriter
     * entities with the LIVELINESS set to MANUAL_BY_PARTICIPANT and it only affects the
     * liveliness of those DataWriter entities. Otherwise, it has no effect.
     *
     * @note Writing data via the write operation on a DataWriter asserts liveliness on
     * the DataWriter itself and its DomainParticipant. Consequently the use of
     * assert_liveliness is only needed if the application is not writing data regularly.
     *
    ...
    /**
     * This operation sets a default value of the Publisher QoS policies which will be
     * used for newly created Publisher entities in the case where the QoS policies are
     * defaulted in the create_publisher operation.
     *
     * This operation will check that the resulting policies are self consistent; if they
     * are not, the operation will have no effect and return false.
     *
     * The special value PUBLISHER_QOS_DEFAULT may be passed to this operation to indicate
     * that the default QoS should be reset back to the initial values the factory would
     * use, that is the values that would be used if the set_default_publisher_qos
    ...
    /**
     * This operation retrieves the default value of the Publisher QoS, that is, the QoS
     * policies which will be used for newly created Publisher entities in the case where
     * the QoS policies are defaulted in the create_publisher operation.
     *
     * The values retrieved get_default_publisher_qos will match the set of values
     * specified on the last successful call to set_default_publisher_qos, or else, if the
     * call was never made, the default values.
     *
    ...
    /**
     * This operation sets a default value of the Subscriber QoS policies that will be
     * used for newly created Subscriber entities in the case where the QoS policies are
     * defaulted in the create_subscriber operation.
     *
     * This operation will check that the resulting policies are self consistent; if they
     * are not, the operation will have no effect and return false.
     *
     * The special value SUBSCRIBER_QOS_DEFAULT may be passed to this operation to
     * indicate that the default QoS should be reset back to the initial values the
     * factory would use, that is the values that would be used if the
     * set_default_subscriber_qos operation had never been called.
     *
    ...
    /**
     * This operation retrieves the default value of the Subscriber QoS, that is, the QoS
     * policies which will be used for newly created Subscriber entities in the case where
     * the QoS policies are defaulted in the create_subscriber operation.
     *
     * The values retrieved get_default_subscriber_qos will match the set of values
     * specified on the last successful call to set_default_subscriber_qos, or else, if
     * the call was never made, the default values.
     *
    ...
    /**
     * This operation sets a default value of the Topic QoS policies which will be used
     * for newly created Topic entities in the case where the QoS policies are defaulted
     * in the create_topic operation.
     *
     * This operation will check that the resulting policies are self consistent; if they
     * are not, the operation will have no effect and return INCONSISTENT_POLICY.
     *
     * The special value TOPIC_QOS_DEFAULT may be passed to this operation to indicate
     * that the default QoS should be reset back to the initial values the factory would
     * use, that is the values that would be used if the set_default_topic_qos operation
    ...
    /**
     * This operation retrieves the default value of the Topic QoS, that is, the QoS
     * policies that will be used for newly created Topic entities in the case where the
     * QoS policies are defaulted in the create_topic operation.
     *
     * The values retrieved get_default_topic_qos will match the set of values specified
     * on the last successful call to set_default_topic_qos, or else, TOPIC_QOS_DEFAULT if
    ...
    /**
     * Retrieves the list of DomainParticipants that have been discovered in the domain
     * and are not "ignored".
     *
     ...
     */
    RTPS_DllAPI ReturnCode_t
    get_discovered_participants(std::vector<InstanceHandle_t> &participant_handles) const;

    /**
     * Retrieves the DomainParticipant data of a discovered not ignored participant.
     *
     * @param[out]  participant_data Reference to the ParticipantBuiltinTopicData object
     * to return the data
     * @param participant_handle InstanceHandle of DomainParticipant to retrieve the data
     * from
     * @return RETCODE_OK if everything correct, PRECONDITION_NOT_MET if participant does
     * not exist
     */
    ...
    /**
     * Retrieves the list of topics that have been discovered in the domain and are not
     * "ignored".
     *
     * @param[out]  topic_handles Reference to the vector where discovered topics will be
     * returned
     * @return RETCODE_OK if everything correct, error code otherwise
     */
    RTPS_DllAPI ReturnCode_t
     ...
     *
     * @param[out]  topic_data Reference to the TopicBuiltinTopicData object to return the
     * data
     * @param topic_handle InstanceHandle of Topic to retrieve the data from
     * @return RETCODE_OK if everything correct, PRECONDITION_NOT_MET if topic does not
     * exist
     */
    RTPS_DllAPI ReturnCode_t
    ...
    /**
     * This operation checks whether or not the given handle represents an Entity that was
     * created from the DomainParticipant.
     *
     * @param a_handle InstanceHandle of the entity to look for.
     * @param recursive The containment applies recursively. That is, it applies both to
     * entities (TopicDescription, Publisher, or Subscriber) created directly using the
     * DomainParticipant as well as entities created using a contained Publisher, or
     * Subscriber as the factory, and so forth. (default: true)
    ...
    /**
     * This operation returns the current value of the time that the service uses to
     * time-stamp data-writes and to set the reception-timestamp for the data-updates it
     * receives.
     *
     ...
     * RERCODE_PRECONDITION_NOT_MET if there is another TypeSupport with the same name and
     ...
     * @return RETCODE_BAD_PARAMETER if the size of the name is 0,
     * RERCODE_PRECONDITION_NOT_MET if there are entities using that TypeSupport and
    ...
    /**
     * This method can be used when using a StaticEndpointDiscovery mechanism different
     * that the one included in FastRTPS, for example when communicating with other
     * implementations. It indicates the Participant that an Endpoint from the XML has
     * been discovered and should be activated.
     *
    ...
    /**
     * Helps the user to solve all dependencies calling internally to the typelookup
     * service and registers the resulting dynamic type. The registration will be perform
     * asynchronously and the user will be notified through the given callback, which
     * receives the type_name as unique argument. If the type is already registered, the
     * function will return true, but the callback will not be called. If the given
     * type_information is enough to build the type without using the typelookup service,
     * it will return true and the callback will be never called.
     *
     * @param type_information
     * @param type_name
     * @param callback
     * @return true if type is already available (callback will not be called). false if
     * type isn't available yet (the callback will be called if negotiation is success,
     * and ignored in other case).
     */
    RTPS_DllAPI ReturnCode_t register_remote_type(
        const fastrtps::types::TypeInformation &type_information,
        const std::string &type_name,
        std::function<void(const std::string &name,
                           const fastrtps::types::DynamicType_ptr type)> &callback);

    /**
     * Register a custom content filter factory, which can be used to create a
     * ContentFilteredTopic.
     *
     * DDS specifies a SQL-like content filter to be used by content filtered topics.
     * If this filter does not meet your filtering requirements, you can register a custom
     * filter factory.
     *
     * To use a custom filter, a factory for it must be registered in the following
     * places:
     *
     * - In any application that uses the custom filter factory to create a
     * ContentFilteredTopic and the corresponding DataReader.
     *
     * - In each application that writes the data to the applications mentioned above.
     *
     * For example, suppose Application A on the subscription side creates a Topic named X
     * and a ContentFilteredTopic named filteredX (and a corresponding DataReader), using
     * a previously registered content filter factory, myFilterFactory. With only that,
     * you will have filtering at the subscription side. If you also want to perform
     * filtering in any application that publishes Topic X, then you also need to register
     * the same definition of the ContentFilterFactory myFilterFactory in that
     * application.
     *
     * Each @c filter_class_name can only be used to register a content filter factory
     * once per DomainParticipant.
     *
     * @param filter_class_name Name of the filter class. Cannot be nullptr, must not
     * exceed 255 characters, and must be unique within this DomainParticipant.
     * @param filter_factory    Factory of content filters to be registered. Cannot be
     * nullptr.
     *
     * @return RETCODE_BAD_PARAMETER if any parameter is nullptr, or the filter_class_name
     * exceeds 255 characters.
     * @return RETCODE_PRECONDITION_NOT_MET if the filter_class_name has been already
     * registered.
     * @return RETCODE_PRECONDITION_NOT_MET if filter_class_name is
    ...
    RTPS_DllAPI ReturnCode_t register_content_filter_factory(
        const char *filter_class_name, IContentFilterFactory *const filter_factory);

    /**
     * Lookup a custom content filter factory previously registered with
     * register_content_filter_factory.
     *
     * @param filter_class_name Name of the filter class. Cannot be nullptr.
     *
     * @return nullptr if the given filter_class_name has not been previously registered
     * on this DomainParticipant. Otherwise, the content filter factory previously
     * registered with the given filter_class_name.
     */
    RTPS_DllAPI IContentFilterFactory *lookup_content_filter_factory(
        const char *filter_class_name);

    /**
     * Unregister a custom content filter factory previously registered with
     * register_content_filter_factory.
     *
     * A filter_class_name can be unregistered only if it has been previously registered
     * to the DomainParticipant with register_content_filter_factory.
     *
     * The unregistration of filter is not allowed if there are any existing
     * ContentFilteredTopic objects that are using the filter.
     *
     * If there is any existing discovered DataReader with the same filter_class_name,
     * filtering on the writer side will be stopped, but this operation will not fail.
     *
     * @param filter_class_name Name of the filter class. Cannot be nullptr.
     *
     * @return RETCODE_BAD_PARAMETER if the filter_class_name is nullptr.
     * @return RERCODE_PRECONDITION_NOT_MET if the filter_class_name has not been
     * previously registered.
     * @return RERCODE_PRECONDITION_NOT_MET if there is any ContentFilteredTopic
