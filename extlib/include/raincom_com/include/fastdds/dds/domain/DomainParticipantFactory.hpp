...
class DomainParticipantFactory
{
  ...
  public:
    ...
    RTPS_DllAPI DomainParticipant *create_participant(
        DomainId_t domain_id, const DomainParticipantQos &qos,
    ...
    RTPS_DllAPI DomainParticipant *create_participant_with_profile(
        DomainId_t domain_id, const std::string &profile_name,
        DomainParticipantListener *listener = nullptr,
        const StatusMask &mask              = StatusMask::all());
     ...
     */
    RTPS_DllAPI DomainParticipant *create_participant_with_profile(
        const std::string &profile_name, DomainParticipantListener *listener = nullptr,
        const StatusMask &mask = StatusMask::all());

    /**
     * This operation retrieves a previously created DomainParticipant belonging to
     * specified domain_id. If no such DomainParticipant exists, the operation will return
     * 'nullptr'. If multiple DomainParticipant entities belonging to that domain_id
    ...
    /**
     * @brief This operation retrieves the default value of the DomainParticipant QoS,
     * that is, the QoS policies which will be used for newly created DomainParticipant
     * entities in the case where the QoS policies are defaulted in the create_participant
     * operation. The values retrieved get_default_participant_qos will match the set of
     * values specified on the last successful call to set_default_participant_qos, or
    ...
    /**
     * @brief This operation sets a default value of the DomainParticipant QoS policies
     * which will be used for newly created DomainParticipant entities in the case where
     * the QoS policies are defaulted in the create_participant operation.
     *
     ...
     *
     * The special value PARTICIPANT_QOS_DEFAULT may be passed to this operation to
     ...
     * @return RETCODE_PRECONDITION_NOT_MET if the participant has active entities,
     * RETCODE_OK if the participant is correctly deleted and RETCODE_ERROR otherwise.
     */
    ...
    /**
     * This operation sets the value of the DomainParticipantFactory QoS policies. These
     * policies control the behavior of the object a factory for entities.
     *
  ...
  protected:
    ...
    static void set_qos(DomainParticipantFactoryQos &to,
                        const DomainParticipantFactoryQos &from, bool first_time);
