...
class Publisher : public DomainEntity
{
  protected:
    ...
    Publisher(DomainParticipant *dp, const PublisherQos &qos = PUBLISHER_QOS_DEFAULT,
  ...
  public:
     ...
     * participant creating this Publisher is not enabled.
     */
    ...
    /**
     * This operation creates a DataWriter. The returned DataWriter will be attached and
     ...
     */
    RTPS_DllAPI DataWriter *create_datawriter(Topic *topic, const DataWriterQos &qos,
    ...
    RTPS_DllAPI DataWriter *create_datawriter_with_profile(
        Topic *topic, const std::string &profile_name,
        DataWriterListener *listener = nullptr,
        const StatusMask &mask       = StatusMask::all());
     ...
     *
     * The delete_datawriter operation must be called on the same Publisher object used to
     * create the DataWriter. If delete_datawriter is called on a different Publisher, the
     * operation will have no effect and it will return false.
     *
     * The deletion of the DataWriter will automatically unregister all instances.
     * Depending on the settings of the WRITER_DATA_LIFECYCLE QosPolicy, the deletion of
     * the DataWriter may also dispose all instances.
     *
     * @param writer DataWriter to delete
     * @return RETCODE_PRECONDITION_NOT_MET if it does not belong to this Publisher,
    ...
    /**
     * This operation retrieves a previously created DataWriter belonging to the Publisher
     * that is attached to a Topic with a matching topic_name. If no such DataWriter
     * exists, the operation will return nullptr.
     *
     * If multiple DataWriter attached to the Publisher satisfy this condition, then the
     * operation will return one of them. It is not specified which one.
     *
     * @param topic_name Name of the Topic
     * @return Pointer to a previously created DataWriter associated to a Topic with the
     * requested topic_name
     */
    ...
    /**
     * @brief Signals the beginning of a set of coherent cache changes using the
     * Datawriters attached to the publisher
     *
     * @return RETCODE_OK if successful, an error code otherwise
     */
     ...
     * reliable DataWriter entities is acknowledged by all matched reliable DataReader
     * entities, or else the duration specified by the max_wait parameter elapses,
     * whichever happens first. A return value of true indicates that all the samples
     * written have been acknowledged by all reliable matched data readers; a return value
     * of false indicates that max_wait elapsed before all the data was acknowledged.
     *
     ...
     * established, RETCODE_OK if the Publisher receives the acknowledgments and
    ...
    /**
     * This operation sets a default value of the DataWriter QoS policies which will be
     * used for newly created DataWriter entities in the case where the QoS policies are
     * defaulted in the create_datawriter operation.
     *
     ...
     *
     * The special value DATAWRITER_QOS_DEFAULT may be passed to this operation to
    ...
    /**
     * This operation returns the default value of the DataWriter QoS, that is, the QoS
     * policies which will be used for newly created DataWriter entities in the case where
     * the QoS policies are defaulted in the create_datawriter operation.
     *
     * The values retrieved by get_default_datawriter_qos will match the set of values
     * specified on the last successful call to set_default_datawriter_qos, or else, if
    ...
    /**
     * This operation retrieves the default value of the DataWriter QoS, that is, the QoS
     * policies which will be used for newly created DataWriter entities in the case where
     * the QoS policies are defaulted in the create_datawriter operation.
     *
     * The values retrieved by get_default_datawriter_qos will match the set of values
