...
/**
 * Class RTPSDomain,it manages the creation and destruction of RTPSParticipant RTPSWriter
 * and RTPSReader. It stores a list of all created RTPSParticipant. It has only static
 * methods.
 * @ingroup RTPS_MODULE
 */
class RTPSDomain
{
  ...
  public:
     ...
     *
     * \post After this call, all the pointers to RTPS entities are invalidated and their
     * use may result in undefined behaviour.
     */
     ...
     * removeRTPSParticipant() or stopAll(), so its use may result in undefined behaviour.
     */
    RTPS_DllAPI static RTPSParticipant *createParticipant(
        uint32_t domain_id, const RTPSParticipantAttributes &attrs,
        RTPSParticipantListener *plisten = nullptr);

    /**
     * @brief Create a RTPSParticipant.
     * @param domain_id DomainId to be used by the RTPSParticipant (80 by default).
     * @param enabled True if the RTPSParticipant should be enabled on creation. False if
     * it will be enabled later with RTPSParticipant::enable()
     * @param attrs RTPSParticipant Attributes.
     * @param plisten Pointer to the ParticipantListener.
     * @return Pointer to the RTPSParticipant.
     *
     ...
     *
     * \warning The returned pointer is invalidated after a call to removeRTPSWriter() or
     * stopAll(), so its use may result in undefined behaviour.
     */
    RTPS_DllAPI static RTPSWriter *createRTPSWriter(RTPSParticipant *p,
    ...
    RTPS_DllAPI static RTPSWriter *createRTPSWriter(
        RTPSParticipant *p, WriterAttributes &watt,
     ...
     */
    RTPS_DllAPI static RTPSWriter *createRTPSWriter(
        RTPSParticipant *p, const EntityId_t &entity_id, WriterAttributes &watt,
        const std::shared_ptr<IPayloadPool> &payload_pool, WriterHistory *hist,
        WriterListener *listen = nullptr);
     ...
     *
     * \warning The returned pointer is invalidated after a call to removeRTPSReader() or
     * stopAll(), so its use may result in undefined behaviour.
     */
    RTPS_DllAPI static RTPSReader *createRTPSReader(RTPSParticipant *p,
    ...
    RTPS_DllAPI static RTPSReader *createRTPSReader(
        RTPSParticipant *p, ReaderAttributes &ratt,
     ...
     */
    RTPS_DllAPI static RTPSReader *createRTPSReader(
        RTPSParticipant *p, const EntityId_t &entity_id, ReaderAttributes &ratt,
        const std::shared_ptr<IPayloadPool> &payload_pool, ReaderHistory *hist,
        ReaderListener *listen = nullptr);
    ...
    /**
     * Creates a RTPSParticipant as default server or client if ROS_MASTER_URI environment
     * variable is set.
