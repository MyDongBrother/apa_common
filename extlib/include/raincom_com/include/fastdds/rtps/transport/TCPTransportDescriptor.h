 ...
 *
 * - \c keep_alive_timeout_ms: time since sending the last keep alive request to consider
 * a connection as broken (in ms).
 *
 ...
 * negotiation.
 *
 * - \c logical_port_increment: increment between logical ports to try during RTCP
 * negotiation.
 *
...
struct TCPTransportDescriptor : public SocketTransportDescriptor
{
    ...
    struct TLSConfig
    {
         ...
         * using the add_verify_mode() member function.
         *
    ...
    //! Time since sending the last keep alive request to consider a connection as broken
    //! (ms)
