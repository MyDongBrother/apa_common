 ...
 *
 * - \c non_blocking_send: do not block on send operations. When it is set to true, send
 * operations will return immediately if the buffer is full, but no error will be returned
 * to the upper layer. This means that the application will behave as if the datagram is
 * sent and lost.
 *
...
struct UDPTransportDescriptor : public SocketTransportDescriptor
{
    ...
    //! Copy assignment
    RTPS_DllAPI UDPTransportDescriptor &operator=(const UDPTransportDescriptor &t) =
        default;
     ...
     *
     * When set to true, calls to send_to() will return immediately if the buffer is full,
     * but no error will be returned to the upper layer. This means that the application
     * will behave as if the datagram is sent but lost (i.e. throughput may be reduced).
     * This value is specially useful on high-frequency best-effort writers.
     *
     * When set to false, calls to send_to() will block until the network buffer has space
     * for the datagram. This may hinder performance on high-frequency writers.
     */
