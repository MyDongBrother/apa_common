...
/**
 * Interface against which to implement a transport layer, decoupled from FastRTPS
 * internals. TransportInterface expects the user to implement a logical equivalence
 * between Locators and protocol-specific "channels". This equivalence can be narrowing:
 * For instance in UDP/IP, a port would take the role of channel, and several different
...
class RTPS_DllAPI TransportInterface
{
  public:
    /**
     * Aside from the API defined here, an user-defined Transport must define a descriptor
     * data type and a constructor that expects a constant reference to such descriptor.
     * e.g:
     *
     * class MyTransport:
     * public:
    ...
    /**
     * Initialize this transport. This method will prepare all the internals of the
     * transport.
     ...
     */
    virtual bool init(const fastrtps::rtps::PropertyPolicy *properties = nullptr) = 0;
     ...
     *   - if a locator from an entry is selected, selector.select is called for that
     * entry
     *
    ...
    virtual bool fillMetatrafficUnicastLocator(
        Locator &locator, uint32_t metatraffic_unicast_port) const = 0;

    //! Configure the initial peer locators list
    virtual bool configureInitialPeerLocator(
        Locator &locator, const fastrtps::rtps::PortParameters &port_params,
  ...
  protected:
    TransportInterface(int32_t transport_kind) : transport_kind_(transport_kind) {}
