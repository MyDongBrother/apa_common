...
class ChainingTransport : public TransportInterface
{
  ...
  public:
    ...
    /*!
     * Initialize the low-level transport. This method will prepare all the internals of
     * the transport.
     * @param properties Optional policy to specify additional parameters of the created
     * transport.
     ...
     * Must report whether the input channel associated to this locator is open. Channels
     * must either be fully closed or fully open, so that "open" and "close" operations
     * are whole and definitive.
     */
     ...
     * Must report whether the given locator is supported by this transport (typically
     * inspecting its "kind" value).
     */
     ...
     * Returns the locator describing the main (most general) channel that can write to
     * the provided remote locator.
     */
     ...
     */
    RTPS_DllAPI bool OpenOutputChannel(SendResourceList &sender_resource_list,
     ...
     * IMPORTANT: It MUST be safe to call this method even during a Receive operation on
     * another thread. You must implement any necessary mutual exclusion and timeout
     * mechanisms to make sure the channel can be closed without damage.
     */
     ...
     */
    RTPS_DllAPI bool getDefaultUnicastLocators(fastrtps::rtps::LocatorList_t &locators,
    ...
    {
        return low_level_transport_->configureInitialPeerLocator(locator, port_params,
                                                                 domainId, list);
    }
    ...
    /**
     * Blocking Send through the specified channel. It may perform operations on the
     output buffer.
     ...
     * @param send_buffer_size Size of the raw data. It will be used as a bounds check for
     the previous argument.
     * It must not exceed the \c sendBufferSize fed to this class during construction.
     * @param destination_locators_begin First iterator of the list of Locators describing
     the remote destinations
     * we're sending to.
     * @param destination_locators_end End iterator of the list of Locators describing the
     remote destinations
     * we're sending to.
    ...
    /*!
     * Blocking Receive from the specified channel. It may perform operations on the input
     buffer.
     ...
     * @param receive_buffer vector with enough capacity (not size) to accommodate a full
     receive buffer. That
     * capacity must not be less than the \c receiveBufferSize supplied to this class
     during construction.
     * @param receive_buffer_size Size of the raw data. It will be used as bounds check
     ...
     * @param[out] remote_locator Locator describing the remote destination we received a
     packet from.
     */
