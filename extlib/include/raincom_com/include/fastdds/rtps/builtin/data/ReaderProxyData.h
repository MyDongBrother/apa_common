...
/**
 * Class ReaderProxyData, used to represent all the information on a Reader (both local
 * and remote) with the purpose of implementing the discovery.
...
class ReaderProxyData
{
  public:
    ...
    void set_remote_locators(const RemoteLocatorList &locators,
                             const NetworkFactory &network, bool use_multicast_locators);
    ...
    RTPS_DllAPI void content_filter(const fastdds::rtps::ContentFilterProperty &filter)
    {
        content_filter_ = filter;
    }

    RTPS_DllAPI void content_filter(fastdds::rtps::ContentFilterProperty &&filter)
    {
    ...
    RTPS_DllAPI void type_id(const TypeIdV1 &other_type_id) { type_id() = other_type_id; }
    ...
    {
        return fastdds::dds::get_proxy_property<SampleIdentity>("PID_CLIENT_SERVER_KEY",
                                                                m_properties);
    }
    ...
    /**
     * Read the information from a CDRMessage_t. The position of the message must be in
     * the beginning on the parameter list.
     ...
     * @return true on success
     */
    RTPS_DllAPI bool readFromCDRMessage(CDRMessage_t *msg, const NetworkFactory &network,
