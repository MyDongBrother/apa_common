 ...
 *implementations It also keeps the Participant Discovery Data and provides interfaces to
 *access it
...
class PDP
{
  ...
  public:
    ...
    virtual ParticipantProxyData *createParticipantProxyData(
        const ParticipantProxyData &p, const GUID_t &writer_guid) = 0;

    /**
     * Force the sending of our local DPD to all remote RTPSParticipants and multicast
     * Locators.
     * @param new_change If true a new change (with new seqNum) is created and sent;If
     * false the last change is re-sent
     ...
     * @param [out] participant_guid  GUID of the ParticipantProxyData where the reader
     * was added.
     * @param [in]  initializer_func  Function to be called in order to set the data of
     * the ReaderProxyData.
     *
     ...
     * @param [out] participant_guid  GUID of the ParticipantProxyData where the writer
     * was added.
     * @param [in]  initializer_func  Function to be called in order to set the data of
     * the WriterProxyData.
     *
    ...
    /**
     * This method gets a copy of a ReaderProxyData object if it is found among the
     * registered RTPSParticipants (including the local RTPSParticipant).
     * @param [in]  reader  GUID_t of the reader we are looking for.
     * @param [out] rdata   Reference to the ReaderProxyData object where data is to be
     * returned.
     * @return True if found.
     */
    ...
    /**
     * This method gets a copy of a WriterProxyData object if it is found among the
     * registered RTPSParticipants (including the local RTPSParticipant).
     * @param [in]  writer  GUID_t of the writer we are looking for.
     * @param [out] wdata   Reference to the WriterProxyData object where data is to be
     * returned.
     * @return True if found.
     */
    ...
    /**
     * This method assigns remote endpoints to the builtin endpoints defined in this
     * protocol. It also calls the corresponding methods in EDP and WLP.
     ...
     * @param remote_guid GuidPrefix_t of the participant whose liveliness is being
     * asserted.
     */
     ...
     * @param guid_prefix The GUID prefix of the participant of which the proxy data is
     * retrieved
  ...
  protected:
     ...
     *
     * @param participant_guid GUID of the participant for which to create the proxy
     * object.
     * @param with_lease_duration indicates whether lease duration event should be
     * created.
     * @param participant_proxy_data The participant proxy data from which the copy is
     * made (if provided)
     *
     * @return pointer to the currently inserted entry, nullptr if allocation limits were
     * reached.
     */
