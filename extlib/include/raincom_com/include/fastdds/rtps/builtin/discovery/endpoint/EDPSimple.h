...
/**
 * Class EDPSimple, implements the Simple Endpoint Discovery Protocol defined in the RTPS
 *specification. Inherits from EDP class.
...
class EDPSimple : public EDP
{
  ...
  public:
    ...
    /**
     * This method assigns the remote builtin endpoints that the remote RTPSParticipant
     * indicates is using to our local builtin endpoints.
    ...
    /**
     * This method generates the corresponding change in the subscription writer and send
     * it to all known remote endpoints.
    ...
    /**
     * This method generates the corresponding change in the publciations writer and send
     * it to all known remote endpoints.
    ...
    /**
     * This methods generates the change disposing of the local Reader and calls the
     * unpairing and removal methods of the base class.
     * @param R Pointer to the RTPSReader object.
     * @return True if correct.
     */
    bool removeLocalReader(RTPSReader *R) override;
    /**
     * This methods generates the change disposing of the local Writer and calls the
     * unpairing and removal methods of the base class.
  ...
  protected:
     ...
     * @param [out] created_change Where the pointer to the created change should be
     * returned.
    ...
    void processPersistentData(t_p_StatefulReader &reader, t_p_StatefulWriter &writer,
                               key_list &demises);
  ...
  private:
    ...
    bool serialize_proxy_data(const ProxyData &data, const t_p_StatefulWriter &writer,
    ...
    bool pairing_remote_writer_with_local_builtin_reader_after_security(
        const GUID_t &local_reader, const WriterProxyData &remote_writer_data) override;

    bool pairing_remote_reader_with_local_builtin_writer_after_security(
        const GUID_t &local_writer, const ReaderProxyData &remote_reader_data) override;
#endif // if HAVE_SECURITY
};
