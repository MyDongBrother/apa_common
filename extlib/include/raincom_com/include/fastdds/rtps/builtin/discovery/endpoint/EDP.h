...
/**
 * Class EDP, base class for Endpoint Discovery Protocols. It contains generic methods
 * used by the two EDP implemented (EDPSimple and EDPStatic), as well as abstract methods
  ...
  public:
    ...
    class MatchingFailureMask : public std::bitset<MATCH_FAILURE_REASON_COUNT>
    {
      public:
        ...
        //! Bit index for matching failing due to inconsistent topic (same topic name but
    ...
    /**
     * Abstract method that assigns remote endpoints when a new RTPSParticipantProxyData
     * is discovered.
    ...
    /**
     * After a new local ReaderProxyData has been created some processing is needed
    ...
    /**
     * After a new local WriterProxyData has been created some processing is needed
     * (depends on the implementation).
    ...
    /**
     * Check the validity of a matching between a local RTPSWriter and a ReaderProxyData
     * object.
    ...
    /**
     * Check the validity of a matching between a local RTPSReader and a WriterProxyData
     * object.
     ...
     * @param [out] incompatible_qos On return will specify all the QoS values that were
     * incompatible (if any).
     * @return True if the two can be matched.
     */
    bool valid_matching(const WriterProxyData *wdata, const ReaderProxyData *rdata,
     ...
     * @return True if the two can be matched.
     */
    bool valid_matching(const ReaderProxyData *rdata, const WriterProxyData *wdata,
                        MatchingFailureMask &reason,
                        fastdds::dds::PolicyMask &incompatible_qos);
     ...
     * @param removed_by_lease Whether the writer is being unpaired due to a participant
     * drop.
     * @return True if correct.
     */
     ...
     * @return True.
     */
    bool pairing_reader_proxy_with_any_local_writer(const GUID_t &participant_guid,
                                                    ReaderProxyData *rdata);

#if HAVE_SECURITY
    bool pairing_reader_proxy_with_local_writer(const GUID_t &local_writer,
     ...
     * @return True.
     */
    bool pairing_writer_proxy_with_any_local_reader(const GUID_t &participant_guid,
                                                    WriterProxyData *wdata);

#if HAVE_SECURITY
    bool pairing_writer_proxy_with_local_reader(const GUID_t &local_reader,
                                                const GUID_t &remote_participant_guid,
                                                WriterProxyData &wdata);
    ...
    const fastdds::dds::SubscriptionMatchedStatus &update_subscription_matched_status(
        const GUID_t &reader_guid, const GUID_t &writer_guid, int change);
