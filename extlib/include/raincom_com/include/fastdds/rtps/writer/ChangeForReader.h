...
/**
 * Struct ChangeForReader_t used to represent the state of a specific change with respect
 * to a specific reader, as well as its relevance.
 *  @ingroup COMMON_MODULE
 */
class ChangeForReader_t
{
  ...
  public:
    ...
    void markFragmentsAsSent(const FragmentNumber_t &sentFragment)
    {
        ...
        // We only use the running window mechanism during the first stage, until all
        // fragments have been delivered once, and we consider the whole change as
        // delivered.
    ...
    {
        // Ignore NACK_FRAG messages during the first stage, until all fragments have been
        // delivered once, and we consider the whole change as delivered.
        if (delivered_)
        {
            if (unsent_fragments_.empty())
            {
            ...
            else
            {
                ...
                unsentFragments.for_each(
                    [this](FragmentNumber_t element) { unsent_fragments_.add(element); });
            }
        }
    }
