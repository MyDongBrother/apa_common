...
/**
 * Class RTPSWriter, manages the sending of data to the readers. Is always associated with
 * a HistoryCache.
 * @ingroup WRITER_MODULE
 */
class RTPSWriter : public Endpoint, public fastdds::statistics::StatisticsWriterImpl
{
  ...
  public:
     ...
     */
    template <typename T>
    CacheChange_t *new_change(T &data, ChangeKind_t changeKind,
                              InstanceHandle_t handle = c_InstanceHandle_Unknown)
    {
        return new_change(
            [data]() -> uint32_t { return (uint32_t)T::getCdrSerializedSize(data); },
            changeKind, handle);
    }

    RTPS_DllAPI CacheChange_t *new_change(
        const std::function<uint32_t()> &dataCdrSerializedSize, ChangeKind_t changeKind,
        InstanceHandle_t handle = c_InstanceHandle_Unknown);
     ...
     */
    RTPS_DllAPI virtual const fastdds::rtps::IReaderDataFilter *reader_data_filter()
        const = 0;

    /**
     * Check if a specific change has been acknowledged by all Readers.
     * Is only useful in reliable Writer. In BE Writers returns false when pending to be
     * sent.
  ...
  protected:
    //! Is the data sent directly or announced by HB and THEN sent to the ones who ask for
    //! it?.
