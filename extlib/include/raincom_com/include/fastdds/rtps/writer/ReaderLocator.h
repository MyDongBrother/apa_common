...
/**
 * Class ReaderLocator, contains information about a remote reader, without saving its
 * state. It also implements RTPSMessageSenderInterface, so it can be used when separate
 * sending is enabled.
...
class ReaderLocator : public RTPSMessageSenderInterface
{
  public:
     ...
     *
     * @return When all the destinations share the same prefix (i.e. belong to the same
     * participant) that prefix is returned. When there are no destinations, or they
     * belong to different participants, c_GuidPrefix_Unknown is returned.
     */
     ...
     *
     * @return a const reference to a vector with the GUID prefix of all destination
     * participants.
     */
     ...
     */
    const std::vector<GUID_t> &remote_guids() const override { return guid_as_vector_; }
    ...
    /**
     * @return The datasharing notifier for this reader or nullptr if the reader is not
     * datasharing.
     */
    ...
    /**
     * Performs datasharing notification of changes on the state of a writer to the reader
