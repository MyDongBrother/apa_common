...
/**
 * ReaderProxy class that helps to keep the state of a specific Reader with respect to the
 * RTPSWriter.
...
class ReaderProxy
{
  public:
     ...
     * @param is_datasharing whether the reader is datasharing compatible with the writer
     * or not.
     */
    ...
    void add_change(
        const ChangeForReader_t &change, bool is_relevant, bool restart_nack_supression,
     ...
     * @return true when the change is irrelevant or has been already acknowledged, false
     * otherwise.
     */
     ...
     * @param[out] gap_seq Return, when it is its first delivery (should be relevant
     * seq_num), the sequence number of the first sequence of the gap [first, seq_num).
     * Otherwise return SequenceNumber_t::unknown().
     * @param[in]  min_seq Minimum sequence number managed by the History. It could be
     * SequenceNumber_t::unknown() if history is empty.
     * @param[out] need_reactivate_periodic_heartbeat Indicates if the heartbeat period
     * event has to be restarted.
     *
     * @return true if the change is marked to be sent. False otherwise.
     */
    bool change_is_unsent(const SequenceNumber_t &seq_num,
                          FragmentNumber_t &next_unsent_frag, SequenceNumber_t &gap_seq,
     ...
     * @param gap_builder RTPSGapBuilder reference uses for adding  each requested change
     * that is irrelevant for the requester.
     * @param[in] min_seq_in_history Minimum SequenceNumber_t in the writer's history. If
     * writer's history is empty, SequenceNumber_t::unknown() is expected.
     ...
     * @param func functor called, if the requester is a local reader, for each changes
     * moved to UNSENT status.
     ...
     * @param restart_nack_supression Whether nack supression event should be restarted or
     * not.
     * @param delivered true if change was able to be delivered to its addressees. false
     * otherwise.
     */
    void from_unsent_to_status(const SequenceNumber_t &seq_num,
                               ChangeForReaderStatus_t status,
                               bool restart_nack_supression, bool delivered = true);
     ...
     * @return true if the reader represented by this proxy expexts inline QOS to be
     * received.
     */
    ...
    /**
     * Called when an ACKNACK is received to set a new value for the count of the last
     * received ACKNACK.
    ...
    bool is_datasharing_reader() const { return locator_info_.is_datasharing_reader(); }
  ...
  private:
     ...
     * @param exact When false, the first change with a sequence number not less than
     * seq_num will be returned. When true, the change with a sequence number value of
     * seq_num will be returned.
