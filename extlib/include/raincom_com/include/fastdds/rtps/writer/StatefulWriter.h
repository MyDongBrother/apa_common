...
/**
 * Class StatefulWriter, specialization of RTPSWriter that maintains information of each
 * matched Reader.
...
class StatefulWriter : public RTPSWriter
{
  ...
  protected:
    ...
    StatefulWriter(RTPSParticipantImpl *impl, const GUID_t &guid,
                   ...
                   fastdds::rtps::FlowController *flow_controller, WriterHistory *hist,
  ...
  private:
    ...
    //! A timed event to mark samples as acknowledget (used only if disable positive ACKs
    //! QoS is enabled)
  ...
  public:
    ...
    /**
     * Indicate the writer that a change has been removed by the history due to some
     * HistoryQos requirement.
    ...
    bool intraprocess_gap(ReaderProxy *reader_proxy, const SequenceNumber_t &seq_num)
    {
    ...
    {
        // we cannot directly pass iterators neither const_iterators to matched_readers_
        // because then the functor would be able to modify ReaderProxy elements
     ...
     */
    void send_heartbeat_to_nts(ReaderProxy &remoteReaderProxy, bool liveliness = false,
                               bool force = false);
     ...
     * @return true when the submessage was destinated to this writer, false otherwise.
     */
    virtual bool process_nack_frag(const GUID_t &writer_guid, const GUID_t &reader_guid,
                                   uint32_t ack_count, const SequenceNumber_t &seq_num,
     ...
     *
     * This method sets a content filter that will be used to check whether a cache change
     * is relevant for a reader or not.
     *
     * @param filter  The content filter to use on this writer. May be @c nullptr to
     * remove the content filter (i.e. treat all samples as relevant).
     */
     ...
     *
     * @param cache_change Pointer to the CacheChange_t that represents the sample which
     * can be sent.
     * @param group RTPSMessageGroup reference uses for generating the RTPS message.
     * @param locator_selector RTPSMessageSenderInterface reference uses for selecting
     * locators. The reference has to be a member of this RTPSWriter object.
  ...
  private:
    ...
    void send_heartbeat_nts_(size_t number_of_readers, RTPSMessageGroup &message_group,
                             bool final, bool liveliness = false);
    ...
    /**
     * Check the StatefulWriter's sequence numbers and add the required GAP messages to
     * the provided message group.
     *
     * @param group     Reference to the Message Group to which the GAP messages are to be
     * added.
     */
