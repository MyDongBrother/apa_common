...
class IPayloadPool
{
  public:
     ...
     *     @li Both different from @c unknown (meaning a reader has received the first
     * fragment of a cache change)
     *
     * @post
     *     @li Field @c cache_change.payload_owner equals this
     *     @li Field @c serializedPayload.data points to a buffer of at least @c size
     * bytes
     ...
     * @returns whether the operation succeeded or not
     *
     * @note @c data and @c data_owner are received as references to accommodate the case
     * where several readers receive the same payload. If the payload has no owner, it
     * means it is allocated on the stack of a reception thread, and a copy should be
     * performed. The pool may decide in that case to point @c data.data to the new copy
     * and take ownership of the payload. In that case, when the reception thread is done
     * with the payload (after all readers have been informed of the received data),
     * method @c release_payload will be called to indicate that the reception thread is
     * not using the payload anymore.
     *
     * @warning @c data_owner can only be changed from @c nullptr to @c this. If a value
     * different from
     * @c nullptr is received it should be left unchanged.
     *
     * @warning @c data fields can only be changed when @c data_owner is @c nullptr. If a
     ...
     * @post
     *     @li Field @c cache_change.payload_owner equals this
     *     @li Field @c cache_change.serializedPayload.data points to a buffer of at least
     * @c data.length bytes
     *     @li Field @c cache_change.serializedPayload.length is equal to @c data.length
     *     @li Field @c cache_change.serializedPayload.max_size is greater than or equal
     * to @c data.length
