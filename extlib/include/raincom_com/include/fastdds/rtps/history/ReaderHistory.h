...
class ReaderHistory : public History
{
  ...
  public:
     ...
     *
     * @param [in]  writer_guid                    GUID of the writer where the change
     * came from.
     * @param [in]  total_payload_size             Total payload size of the incoming
     * change.
     * @param [in]  unknown_missing_changes_up_to  The number of changes from the same
     * writer with a lower sequence number that could potentially be received in the
     * future.
     * @param [out] will_never_be_accepted         When the method returns @c false, this
     * parameter will inform whether the change could be accepted in the future or not.
     *
     * @pre change should not be present in the history
     *
     * @return Whether a call to received_change will succeed when called with the same
     * arguments.
     */
    RTPS_DllAPI virtual bool can_change_be_added_nts(const GUID_t &writer_guid,
     ...
     * In this implementation this method just calls add_change. The user can overload
     * this method in case he needs to perform additional checks before adding the change.
     * @param change Pointer to the change
     * @param unknown_missing_changes_up_to The number of changes from the same writer
     * with a lower sequence number that could potentially be received in the future.
    ...
    /**
     * Called when a fragmented change is received completely by the Subscriber. Will find
     * its instance and store it.
     ...
     *
     * This method will remove all the changes on the history that came from the writer
     * being unmatched and which have not yet been notified to the user.
     *
     * @param writer_guid        GUID of the writer being unmatched.
     * @param last_notified_seq  Last sequence number from the specified writer that was
     * notified to the user.
     */
    RTPS_DllAPI virtual void writer_unmatched(const GUID_t &writer_guid,
                                              const SequenceNumber_t &last_notified_seq);

  protected:
    RTPS_DllAPI bool do_reserve_cache(CacheChange_t **change, uint32_t size) override;

    RTPS_DllAPI void do_release_cache(CacheChange_t *ch) override;

    template <typename Pred>
    inline void remove_changes_with_pred(Pred pred)
    {
