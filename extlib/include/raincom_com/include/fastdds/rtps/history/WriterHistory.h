...
class WriterHistory : public rtps::History
{
  ...
  protected:
     ...
     * filled by this method. Its @c wparams will be filled from parameter @c wparams.
     * @param [in,out] wparams        On input, it holds the WriteParams to be copied into
     * @c a_change. On output, will be filled with the sample identity assigned to @c
     * a_change.
     * @param [in] max_blocking_time  Maximum time point the writer is allowed to be
     * blocked till the change is put into the wire or the sending queue.
     *
     ...
     * @c a_change. On output, will be filled with the sample identity assigned to @c
     * a_change.
     * @param [in] pre_commit         Functor called after @c a_change has been added to
     * the history, and its information has been filled, but before the writer is notified
     * of the insertion.
     * @param [in] max_blocking_time  Maximum time point the writer is allowed to be
     * blocked till the change is put into the wire or the sending queue.
     *
        ...
        std::chrono::time_point<std::chrono::steady_clock> max_blocking_time)
    {
        if (mp_writer == nullptr || mp_mutex == nullptr)
        {
            logError(RTPS_WRITER_HISTORY,
                     "You need to create a Writer with this History before adding any "
                     "changes");
            return false;
        }
  ...
  private:
     ...
     *
     * @param [in,out] a_change  The change to be added.
     *                           Its @c sequenceNumber and sourceTimestamp will be filled
     * by this method. Its @c wparams will be filled from parameter @c wparams.
     * @param [in,out] wparams   On input, it holds the WriteParams to be copied into @c
     * a_change. On output, will be filled with the sample identity assigned to @c
     * a_change.
     *
     * @return whether @c a_change could be added to the history.
     */
     ...
     * Depending on the publish mode, it will be directly sent to the wire, or put into a
     * sending queue.
     *
