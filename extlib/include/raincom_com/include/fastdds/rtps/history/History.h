...
class History
{
  ...
  public:
    ...
    RTPS_DllAPI inline bool reserve_Cache(
        CacheChange_t **change, const std::function<uint32_t()> &calculateSizeFunc)
    {
    ...
    RTPS_DllAPI inline void release_Cache(CacheChange_t *ch) { do_release_cache(ch); }
     ...
     * Derived classes have more info on how to identify univocally a change and should
     * override.
     ...
     * @return iterator to the next CacheChange_t or end iterator.
     */
    RTPS_DllAPI iterator remove_change(const_iterator removal, bool release = true)
    {
        std::lock_guard<RecursiveTimedMutex> guard(*mp_mutex);
     ...
     */
    RTPS_DllAPI inline uint32_t getTypeMaxSerialized() { return m_att.payloadMaxSize; }
    ...
    const_iterator get_change_nts(const SequenceNumber_t &seq, const GUID_t &guid,
                                  CacheChange_t **change, const_iterator hint) const;
  ...
  protected:
    ...
    //! Variable to know if the history is full without needing to block the History
    //! mutex.
    ...
    RTPS_DllAPI virtual bool do_reserve_cache(CacheChange_t **change, uint32_t size) = 0;
