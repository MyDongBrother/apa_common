...
/**
 * RAII object that encapsulates the Send operation over one chanel in an unknown
 * transport. A Sender resource is always univocally associated to a transport channel;
 * the act of constructing a Sender Resource opens the channel and its destruction closes
 * it.
...
class SenderResource
{
  public:
     ...
     * @param max_blocking_time_point If transport supports it then it will use it as
     * maximum blocking time.
     * @return Success of the send operation.
     */
    bool send(const octet *data, uint32_t dataLength,
     ...
     */
    virtual void add_locators_to_list(LocatorList_t &locators) const { (void)locators; }
