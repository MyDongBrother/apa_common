...
class RTPS_DllAPI RTPSParticipant
{
  ...
  public:
    ...
    //	//!Method to loose the next change (ONLY FOR TEST). //TODO remove this method
    //because is only for testing 	void loose_next_change();

    //! Stop the RTPSParticipant announcement period. //TODO remove this method because is
    //! only for testing
    void stopRTPSParticipantAnnouncement();

    //! Reset the RTPSParticipant announcement period. //TODO remove this method because
    //! is only for testing
     ...
     */
    bool registerReader(
        RTPSReader *Reader, const TopicAttributes &topicAtt, const ReaderQos &rqos,
    ...
    /**
     * @brief Fills a new entityId if set to unknown, or checks if a entity already exists
     * with that entityId in other case.
     * @param entityId to check of fill. If filled, EntityKind will be "vendor-specific"
     * (0x01)
     ...
     */
    bool add_statistics_listener(std::shared_ptr<fastdds::statistics::IListener> listener,
                                 uint32_t kind);
