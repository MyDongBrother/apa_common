...
class CryptoTransform
{
  public:
     ...
     * @param extra_inline_qos (out) Contains additional parameters to be added to the
     * inlineQos of the submessage
    ...
    virtual bool encode_serialized_payload(
        SerializedPayload_t &encoded_payload, std::vector<uint8_t> &extra_inline_qos,
     ...
     * @param receiving_datareader_crypto_list Crypto of the datareaders the message is
     * aimed at
     * @param exception (out) Security exception
     * @return TRUE is successful
     */
    virtual bool encode_datawriter_submessage(
        CDRMessage_t &encoded_rtps_submessage, const CDRMessage_t &plain_rtps_submessage,
        DatawriterCryptoHandle &sending_datawriter_crypto,
     ...
     * @param receiving_datawriter_crypto_list List with Crypto of the intended datawriter
     * recipients
     * @param exception (out) Security exception
     * @return TRUE if successful
     */
     ...
     * @param receiving_crypto_list Crypto of the Partipants the message is intended
     * towards
     * @param exception (out) Security expcetion
     * @return TRUE if successful
     */
    virtual bool encode_rtps_message(
        CDRMessage_t &encoded_rtps_message, const CDRMessage_t &plain_rtps_message,
        ParticipantCryptoHandle &sending_crypto,
        std::vector<std::shared_ptr<ParticipantCryptoHandle>> &receiving_crypto_list,
        SecurityException &exception) = 0;

    /**
     * Reverses the transformation performed by encode_rtps_message. Decrypts the contents
     * and veryfies MACs or digital signatures.
     ...
     * @return TRUE is successful
     */
    virtual bool decode_rtps_message(CDRMessage_t &plain_buffer,
    ...
    /**
     * Determines whether the secure submessage comes from a datawriter or a data reader
     * and extracts the required CryptoHandle to decode it.
     ...
     * @param secure_submessage_category (out) Specifies whether the message comes from a
     * datawriter or from a datareader
    ...
    virtual bool preprocess_secure_submsg(
        ...
        ParticipantCryptoHandle &sending_crypto, SecurityException &exception) = 0;
    ...
    virtual bool decode_datawriter_submessage(
        CDRMessage_t &plain_rtps_submessage, CDRMessage_t &encoded_rtps_submessage,
    ...
    virtual bool decode_serialized_payload(
        SerializedPayload_t &plain_payload, const SerializedPayload_t &encoded_payload,
