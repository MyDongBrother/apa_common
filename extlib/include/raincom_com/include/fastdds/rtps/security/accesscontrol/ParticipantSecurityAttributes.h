...
struct PluginParticipantSecurityAttributes
{
    ...
    explicit PluginParticipantSecurityAttributes(
        ...
        : is_rtps_encrypted(
              (mask & PLUGIN_PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_RTPS_ENCRYPTED) !=
              0),
          ...
          is_liveliness_encrypted(
              (mask &
               PLUGIN_PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_LIVELINESS_ENCRYPTED) != 0),
...
struct ParticipantSecurityAttributes
{
    ...
    explicit ParticipantSecurityAttributes(const ParticipantSecurityAttributesMask mask)
        : allow_unauthenticated_participants(false),
          is_access_protected(true),
          is_rtps_protected(
              (mask & PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_RTPS_PROTECTED) != 0),
          is_discovery_protected(
              (mask & PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_DISCOVERY_PROTECTED) != 0),
          is_liveliness_protected(
              (mask & PARTICIPANT_SECURITY_ATTRIBUTES_FLAG_IS_LIVELINESS_PROTECTED) != 0),
          plugin_participant_attributes(0UL)
    {
    }
