...
struct PluginEndpointSecurityAttributes
{
    ...
    explicit PluginEndpointSecurityAttributes(
        ...
        : is_submessage_encrypted(
              (mask & PLUGIN_ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_SUBMESSAGE_ENCRYPTED) !=
              0),
          ...
          is_payload_encrypted(
              (mask & PLUGIN_ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_PAYLOAD_ENCRYPTED) != 0)
    {
    }
    ...
    explicit EndpointSecurityAttributes(const EndpointSecurityAttributesMask mask)
        : is_read_protected(
          ...
          is_discovery_protected(
              (mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_DISCOVERY_PROTECTED) != 0),
          is_liveliness_protected(
              (mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_LIVELINESS_PROTECTED) != 0),
          is_submessage_protected(
              (mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_SUBMESSAGE_PROTECTED) != 0),
          is_payload_protected(
              (mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_PAYLOAD_PROTECTED) != 0),
          is_key_protected((mask & ENDPOINT_SECURITY_ATTRIBUTES_FLAG_IS_KEY_PROTECTED) !=
                           0),
          plugin_endpoint_attributes(0UL)
    {
    }
