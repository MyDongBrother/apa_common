 ...
 */
class RTPSReader : public Endpoint, public fastdds::statistics::StatisticsReaderImpl
{
  ...
  public:
     ...
     * @param removed_by_lease Whether the writer is being unmatched due to a participant
     * drop.
     * @return True if correctly removed.
     */
    RTPS_DllAPI virtual bool matched_writer_remove(const GUID_t &writer_guid,
     ...
     */
    RTPS_DllAPI virtual bool processDataFragMsg(CacheChange_t *change,
                                                uint32_t sampleSize,
     ...
     * @return true if the reader accepts messages from the.
     */
    RTPS_DllAPI virtual bool processGapMsg(const GUID_t &writerGUID,
                                           const SequenceNumber_t &gapStart,
                                           const SequenceNumberSet_t &gapList) = 0;

    /**
     * Method to indicate the reader that some change has been removed due to HistoryQos
     * requirements.
     * @param change Pointer to the CacheChange_t.
     * @param prox Pointer to the WriterProxy.
     * @return True if correctly removed.
     */
    RTPS_DllAPI virtual bool change_removed_by_history(CacheChange_t *change,
                                                       WriterProxy *prox = nullptr) = 0;

    /**
     * Get the associated listener, secondary attached Listener in case it is of compound
     * type
     ...
     * If the RTPSReader does not belong to the built-in protocols it switches out the old
     * one. If it belongs to the built-in protocols, it sets the new ReaderListener
     * callbacks to be called after the built-in ReaderListener ones.
  ...
  protected:
     ...
     * @param guid GUID of the remote writer
     * @param persistence_guid Persistence GUID of the remote writer
     * @param removed_by_lease Whether the GUIDs are being removed due to a participant
     * drop.
     */
    void remove_persistence_guid(const GUID_t &guid, const GUID_t &persistence_guid,
    ...
    /*!
     * @brief Search if there is a CacheChange_t, giving SequenceNumber_t and writer
     * GUID_t, waiting to be completed because it is fragmented.
     ...
     * @param change If a CacheChange_t was found, this argument will fill with its
