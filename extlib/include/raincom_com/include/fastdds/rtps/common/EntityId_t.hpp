...
struct RTPS_DllAPI EntityId_t
{
     ...
     */
    EntityId_t(const EntityId_t &id) { memcpy(value, id.value, size); }

    /*!
     * @brief Move constructor
     */
    EntityId_t(EntityId_t &&id) { memmove(value, id.value, size); }
...
#if !FASTDDS_IS_BIG_ENDIAN_TARGET
        res = (res >> 24) | (0x0000ff00 & (res >> 8)) | (0x00ff0000 & (res << 8)) |
              (res << 24);
#endif // if !FASTDDS_IS_BIG_ENDIAN_TARGET
    ...
    static EntityId_t unknown() { return EntityId_t(); }
...
inline bool operator!=(const EntityId_t &id1, const EntityId_t &id2)
{
    ...
    // NOTE: this could be done comparing the entities backwards (starting in [3]) as it
...
{
    output << std::hex;
    output << (int)enI.value[0] << "." << (int)enI.value[1] << "." << (int)enI.value[2]
           << "." << (int)enI.value[3];
...
#if HAVE_SECURITY
const EntityId_t sedp_builtin_publications_secure_writer =
    ENTITYID_SEDP_BUILTIN_PUBLICATIONS_SECURE_WRITER;
const EntityId_t sedp_builtin_publications_secure_reader =
    ENTITYID_SEDP_BUILTIN_PUBLICATIONS_SECURE_READER;
const EntityId_t sedp_builtin_subscriptions_secure_writer =
    ENTITYID_SEDP_BUILTIN_SUBSCRIPTIONS_SECURE_WRITER;
const EntityId_t sedp_builtin_subscriptions_secure_reader =
...
const EntityId_t c_EntityId_WriterLivelinessSecure =
    ENTITYID_P2P_BUILTIN_PARTICIPANT_MESSAGE_SECURE_WRITER;
const EntityId_t c_EntityId_ReaderLivelinessSecure =
...
struct hash<eprosima::fastrtps::rtps::EntityId_t>
{
    std::size_t operator()(const eprosima::fastrtps::rtps::EntityId_t &k) const
    {
        return (static_cast<size_t>(k.value[0]) << 16) |
               (static_cast<size_t>(k.value[1]) << 8) | static_cast<size_t>(k.value[2]);
    }
};
