...
/**
 * Adapter class that provides a LocatorsIterator interface from a
...
class Locators : public LocatorsIterator
{
  public:
    ...
    const Locator &operator*() const { return (*it_); }
...
class LocatorList
{
  public:
     ...
     */
    RTPS_DllAPI LocatorListConstIterator begin() const { return m_locators.begin(); }

    /**
     * @brief Return a constant iterator to the end.
     *
     * @return LocatorListConstIterator iterator to the element following the last
     * element.
     */
    RTPS_DllAPI LocatorListConstIterator end() const { return m_locators.end(); }
     ...
     *        If the current size is greater than @c num, the container is reduced to its
     * first @c num locators. If the current size is less than count, additional
     * default-inserted locators are appended.
     *
 ...
 *        Each individual locator within the list must follow the serialization format
 * explained in the locator insertion operator.
 *
 ...
 * @return \c std::ostream& Reference to the output stream with the serialized locator
 * list appended.
 */
 ...
 *        Each individual locator within the list must follow the deserialization format
 * explained in the locator extraction operator.
 *
