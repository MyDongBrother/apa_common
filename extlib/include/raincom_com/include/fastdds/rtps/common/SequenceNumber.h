...
struct RTPS_DllAPI SequenceNumber_t
{
    ...
    SequenceNumber_t &operator+=(int inc) noexcept
    {
        ...
        if (low < aux_low)
        {
            // Being the type of the parameter an 'int', the increment of 'high' will be
            // as much as 1.
            assert(std::numeric_limits<decltype(high)>::max() > high);
            ++high;
        }
                                  ...
                                  const uint32_t inc) noexcept
{
    ...
    if (inc > seq.low)
    {
        // Being the type of the parameter an 'uint32_t', the decrement of 'high' will be
        // as much as 1.
        assert(0 < res.high);
        --res.high;
    }
                                  ...
                                  const uint32_t inc) noexcept
{
    ...
    if (res.low < seq.low)
    {
        // Being the type of the parameter an 'uint32_t', the increment of 'high' will be
        // as much as 1.
...
inline std::ostream &operator<<(std::ostream &output, const SequenceNumberSet_t &sns)
{
    ...
    sns.for_each([&output](SequenceNumber_t it) { output << it.to64long() << "-"; });
