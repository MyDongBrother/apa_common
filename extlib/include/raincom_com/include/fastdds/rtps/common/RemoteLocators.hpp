...
struct RemoteLocatorList
{
    ...
    RemoteLocatorList(size_t max_unicast_locators, size_t max_multicast_locators)
        : unicast(ResourceLimitedContainerConfig::fixed_size_configuration(
              max_unicast_locators)),
          multicast(ResourceLimitedContainerConfig::fixed_size_configuration(
    ...
    RemoteLocatorList(const RemoteLocatorList &other)
        : unicast(other.unicast), multicast(other.multicast)
    {
    }
     ...
     * @param other RemoteLocatorList to copy data from.
     *
     * @remarks Using the assignment operator is different from copy-constructing as in
     * the first case the configuration with the maximum number of locators is not copied.
     * This means that, for two lists with different maximum number of locators, the
     * expression `(a = b) == b` may not be true.
     */
     ...
     *
     * If the locator already exists in the unicast list, or the maximum number of unicast
     * locators has been reached, the new locator is silently discarded.
     *
     ...
     *
     * If the locator already exists in the multicast list, or the maximum number of
     * multicast locators has been reached, the new locator is silently discarded.
     *
...
/*
 * multicast max_size , multicast size , unicast max_size , unicast size ( locator[0] ,
 * locator[1] , ... )
 */
                                ...
                                const RemoteLocatorList &remote_locators)
{
    ...
    if (!remote_locators.multicast.empty())
    {
        ...
        for (auto it = remote_locators.multicast.begin() + 1;
             it != remote_locators.multicast.end(); ++it)
        {
            output << "," << *it;
        }
        output << "]";
    ...
    if (!remote_locators.unicast.empty())
    {
        ...
        for (auto it = remote_locators.unicast.begin() + 1;
             it != remote_locators.unicast.end(); ++it)
        {
            output << "," << *it;
        }
        output << "]";
