...
struct RTPS_DllAPI CacheChange_t
{
    ...
    /*!
     * Copy a different change into this one. All the elements are copied, included the
     * data, allocating new memory.
     ...
     */
    bool is_fully_assembled() { return first_missing_fragment_ >= fragment_count_; }
     ...
     *
     * @remarks Parameter create_fragment_list should only be true when receiving the
     * first fragment of a change.
     */
    void setFragmentSize(uint16_t fragment_size, bool create_fragment_list = false)
    {
        ...
        if (fragment_size > 0)
        {
            ...
            {
                // Keep index of next fragment on the payload portion at the beginning of
                // each fragment. Last fragment will have fragment_count_ as 'next
                // fragment index'
                size_t offset = 0;
  ...
  private:
    ...
    void set_next_missing_fragment(uint32_t fragment_index, uint32_t next_fragment_index)
    {
        uint32_t *ptr = next_fragment_pointer(fragment_index);
     ...
     */
    bool received_fragments(uint32_t initial_fragment, uint32_t num_of_fragments)
    {
            ...
            else
            {
                ...
                while (current_frag < initial_fragment)
                {
                    ...
                    if (next_frag >= initial_fragment)
                    {
                        ...
                        {
                            set_next_missing_fragment(current_frag,
                                                      next_missing_fragment);
                        }
                        break;
                    }
