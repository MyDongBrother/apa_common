...
/**
 * Class PortParameters, to define the port parameters and gains related with the RTPS
 * protocol.
  ...
  public:
    ...
    inline uint32_t getMulticastPort(uint32_t domainId) const
    {
        ...
        if (port > 65535)
        {
            ...
            std::cout << "Calculated port number is too high. Probably the domainId is "
                         "over 232 "
    ...
    inline uint32_t getUnicastPort(uint32_t domainId, uint32_t RTPSParticipantID) const
    {
        ...
        {
            logError(RTPS,
                     "Calculated port number is too high. Probably the domainId is over "
                     "232, there are "
