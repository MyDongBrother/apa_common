...
/**
 * A class used for the efficient selection of locators when sending data to multiple
 * entities.
 *
 * Algorithm:
...
class LocatorSelector
{
  public:
     ...
     *
     * @param entries_allocation Allocation configuration regarding the number of remote
     * entities.
     */
    ...
    bool remove_entry(const GUID_t &guid)
    {
        return entries_.remove_if(
            [&guid](LocatorSelectorEntry *entry) { return entry->remote_guid == guid; });
    }
    ...
    class iterator : public LocatorsIterator
    {
        ...
        void go_to_next_entry()
        {
            ...
            {
                LocatorSelectorEntry *entry = locator_selector_.entries_.at(
                    locator_selector_.selections_[current_.selections_index]);
      ...
      public:
        ...
        explicit iterator(const LocatorSelector &locator_selector, Position index_pos)
        ...
        iterator &operator++()
        {
            ...
            if (current_.state_multicast_done)
            {
                ...
                {
                    current_.locator =
                        &entry->unicast[entry->state.unicast.at(current_.state_index)];
                }
            }
            else // Index at multicast locators
            {
                ...
                if (++current_.state_index >= entry->state.multicast.size())
                {
                    ...
                    else // current unicast locator
                    {
                        current_.locator =
                            &entry
                                 ->unicast[entry->state.unicast.at(current_.state_index)];
                    }
                }
                else // current multicast locator
                {
                    current_.locator =
                        &entry
                             ->multicast[entry->state.multicast.at(current_.state_index)];
                }
            }
        ...
        bool operator!=(const LocatorsIterator &other) const { return !(*this == other); }
        ...
        pointer operator->() const { return current_.locator; }

        reference operator*() const { return *current_.locator; }
    };

    iterator begin() const { return iterator(*this, iterator::Position::Begin); }

    iterator end() const { return iterator(*this, iterator::Position::End); }
