...
/**
 * Class RemoteServerAttributes, to define the attributes of the Discovery Server
 * Protocol.
...
/* Environment variable to specify a semicolon-separated list of UDPv4 locators (ip:port)
 * that define remote server locators. The position in the list is used as a "server id"
 * to extrapolate the server's GUID prefix. For the variable to take any effect, the
 * following pre-conditions must be met:
 *    1. The server's GUID prefix must be compliant with the schema
 *       "44.53.<server_id_in_hex>.5f.45.50.52.4f.53.49.4d.41", which is the schema
 * followed by the prefixes generated when creating server using fastdds cli, being
 * DEFAULT_ROS2_SERVER_GUIDPREFIX the prefix for ID=0.
 ...
 * DEFAULT_ROS2_MASTER_URI list to expand the list of remote servers.
 */
 ...
 */
RTPS_DllAPI bool load_environment_server_info(std::string list,
                                              RemoteServerList_t &attributes);

/**
 * Retrieves a semicolon-separated list of locators from DEFAULT_ROS2_MASTER_URI
 * environment variable, and populates a RemoteServerList_t mapping list position to
 * default guid.
 *
 * The environment variable can be read from an environment file (which allows runtime
 * modification of the remote servers list) or directly from the environment. The value
 * contained in the file takes precedence over the environment value (if both are set).
 * This is to avoid conflicts because only new servers can be added to the list
 * (containing thus all the previously known servers).
 *
 * @param[out] attributes reference to a RemoteServerList_t to populate.
 ...
 * @return The value of environment variable DEFAULT_ROS2_MASTER_URI. Empty string if the
 ...
 */
RTPS_DllAPI bool get_server_client_default_guidPrefix(int id,
