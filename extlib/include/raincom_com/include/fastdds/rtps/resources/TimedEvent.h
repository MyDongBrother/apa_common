 ...
 *
 * In the construct you can set the callback to be called when the expiration time
 expires.
 * @code
 ...
 * - The returned value tells if the event has to be scheduled again or not. true value
 tells to reschedule the event.
 *   false value doesn't.
 *
 * Usually the callback will call a method of the owner of the event. So the callback
 surely accesses internal data that
 * object. Then you have to be aware of deleting the event before deleting any other
 attribute of the object. Here are
 * explained two cases:
 *
 * - The event is an attribute of the class. Then it has to be declared as the last member
 of the class. Then we assure
 ...
 * @endcode
 *
 * - The class has a pointer to the event (TimedEvent is created in heap). Then the
 pointer has to be the first one on
...
class TimedEvent
{
  public:
     ...
     * When updating the interval, the timer is not restarted and the new interval will
     * only be used the next time you call restart_timer().
     *
