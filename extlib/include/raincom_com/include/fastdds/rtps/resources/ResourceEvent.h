...
class ResourceEvent
{
  public:
    ...
    /*!
     * @brief This method removes a TimedEventImpl object in case it is waiting to be
     * processed by ResourceEvent's internal thread.
     *
     * This method has to be called before deleting the TimedEventImpl object.
     * This method cancels any operation of the timer.
     * Then it avoids the situation of the execution thread calling the event handler when
     * it was previously removed.
     * @param event TimedEventImpl object that will be deleted and we have to be sure all
     * its operations are cancelled.
     */
    void unregister_timer(TimedEventImpl *event);

    /*!
     * @brief This method notifies to ResourceEvent that the TimedEventImpl object has
  ...
  private:
    ...
    //! Flag used to allow a thread to manipulate the timer collections when the execution
    //! thread is not using them.
