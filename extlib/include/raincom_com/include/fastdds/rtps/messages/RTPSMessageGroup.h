...
class RTPSMessageGroup
{
  public:
    ...
    /*!
     * Exception thrown when a operation exceeds the maximum bytes this object can process
     ...
     * @param internal_buffer true indicates this object to allocate its own buffer. false
     * indicates to get a buffer from the participant.
     */
    ...
    bool add_heartbeat(const SequenceNumber_t &first_seq,
                       const SequenceNumber_t &last_seq, Count_t count, bool is_final,
     ...
     */
    bool add_gap(const SequenceNumber_t &gap_initial_sequence,
                 const SequenceNumberSet_t &gap_bitmap, const GUID_t &reader_guid);
     ...
     * @return True when message was added to the group.
     */
    bool add_nackfrag(const SequenceNumber_t &seq_number, FragmentNumberSet_t fn_state,
                      int32_t count);
     ...
     *
     * @param endpoint Pointer to next Endpoint sender. nullptr resets object to initial
     * state.
     * @param msg_sender Pointer to the RTPSMessageSenderInterface will be used to send
     * next RTPS messages.. nullptr resets object to initial state.
