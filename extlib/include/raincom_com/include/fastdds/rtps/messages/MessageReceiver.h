...
class MessageReceiver
{
  ...
  private:
    ...
    std::function<void(const EntityId_t &, CacheChange_t &)>
        process_data_message_function_;
    //! Function used to process a received fragment message
    std::function<void(const EntityId_t &, CacheChange_t &, uint32_t, uint32_t, uint16_t)>
    ...
    void findAllReaders(const EntityId_t &readerID, const Functor &callback) const;
     ...
     * @return
     */
    bool proc_Submsg_Data(CDRMessage_t *msg, SubmessageHeader_t *smh) const;
    bool proc_Submsg_DataFrag(CDRMessage_t *msg, SubmessageHeader_t *smh) const;
    bool proc_Submsg_Heartbeat(CDRMessage_t *msg, SubmessageHeader_t *smh) const;
    bool proc_Submsg_Acknack(CDRMessage_t *msg, SubmessageHeader_t *smh) const;
    bool proc_Submsg_Gap(CDRMessage_t *msg, SubmessageHeader_t *smh) const;
    ...
    bool proc_Submsg_NackFrag(CDRMessage_t *msg, SubmessageHeader_t *smh) const;
    bool proc_Submsg_HeartbeatFrag(CDRMessage_t *msg, SubmessageHeader_t *smh) const;
    ///@}
    ...
    ///@{
#if HAVE_SECURITY
    void process_data_fragment_message_with_security(const EntityId_t &reader_id,
                                                     CacheChange_t &change,
    ...
    void process_data_fragment_message_without_security(const EntityId_t &reader_id,
                                                        CacheChange_t &change,
                                                        uint32_t sample_size,
                                                        uint32_t fragment_starting_num,
                                                        uint16_t fragments_in_submessage);
    ...
    /**
     * Looks for the statistics specific submessage and notifies statistics related to the
     * received message.
     *
     * @param [in] source_locator Locator indicating the sending address.
     * @param [in] reception_locator Locator indicating the listening address.
