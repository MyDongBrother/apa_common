...
/**
 * Namespace CDRMessage, contains inline methods to initialize CDRMessage_t and add or
 ...
 * Methods to read different data types from a CDR message. Pointers to the message and to
 * the data types are provided. The read position is updated in the message. It fails if
 * you attempt to read outside the boundaries of the message.
...
inline bool read_array_with_max_size(CDRMessage_t *msg, octet *arr, size_t max_size);
...
inline bool readParticipantGenericMessage(CDRMessage_t *msg,
                                          security::ParticipantGenericMessage &message);
///@}

/**
 * Initialize given CDR message with default size. It frees the memory already allocated
 * and reserves new one.
...
/**
 * Append given CDRMessage to existing CDR Message. Joins two messages into the first one
 *if it has space.
 ...
 * Methods to add different data types to a CDR message. Pointers to the message and to
 * the data types are provided. The write position is updated in the message. It fails if
 * you attempt to write outside the boundaries of the message.
...
inline bool addMessageIdentity(CDRMessage_t *msg,
