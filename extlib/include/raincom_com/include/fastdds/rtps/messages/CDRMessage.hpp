...
inline bool CDRMessage::addSequenceNumber(CDRMessage_t *msg, const SequenceNumber_t *sn)
{
...
inline bool CDRMessage::addFragmentNumberSet(CDRMessage_t *msg, FragmentNumberSet_t *fns)
{
...
inline bool CDRMessage::addPropertySeq(CDRMessage_t *msg, const PropertySeq &properties)
{
    ...
    if (msg->pos + 4 <= msg->max_size)
    {
        ...
        {
            returnedValue = true;
            for (auto it = properties.begin(); returnedValue && it != properties.end();
                 ++it)
            {
                if (it->propagate())
                {
...
inline bool CDRMessage::addBinaryPropertySeq(CDRMessage_t *msg,
...
inline bool CDRMessage::readBinaryPropertySeq(CDRMessage_t *msg,
...
inline bool CDRMessage::readMessageIdentity(CDRMessage_t *msg,
                                            security::MessageIdentity &message_identity)
{
    assert(msg);

    if (!CDRMessage::readData(msg, message_identity.source_guid().guidPrefix.value,
                              GuidPrefix_t::size))
    {
        return false;
    }
    ...
    CDRMessage_t *msg, const security::ParticipantGenericMessage &message)
{
    ...
    if (!CDRMessage::addData(msg, message.destination_participant_key().guidPrefix.value,
                             GuidPrefix_t::size))
    {
        return false;
    }
    if (!CDRMessage::addData(msg, message.destination_participant_key().entityId.value,
                             EntityId_t::size))
    {
        return false;
    }
    if (!CDRMessage::addData(msg, message.destination_endpoint_key().guidPrefix.value,
                             GuidPrefix_t::size))
    {
        return false;
    }
    if (!CDRMessage::addData(msg, message.destination_endpoint_key().entityId.value,
                             EntityId_t::size))
    {
        return false;
    }
    ...
    CDRMessage_t *msg, security::ParticipantGenericMessage &message)
{
    ...
    if (!CDRMessage::readData(msg, message.destination_participant_key().guidPrefix.value,
                              GuidPrefix_t::size))
    {
        return false;
    }
    if (!CDRMessage::readData(msg, message.destination_participant_key().entityId.value,
                              EntityId_t::size))
    {
        return false;
    }
    if (!CDRMessage::readData(msg, message.destination_endpoint_key().guidPrefix.value,
                              GuidPrefix_t::size))
    {
        return false;
    }
    if (!CDRMessage::readData(msg, message.destination_endpoint_key().entityId.value,
                              EntityId_t::size))
    {
        return false;
    }
