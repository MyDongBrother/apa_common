 ...
 *
 * This template class holds an unordered collection of elements using a std::vector or a
 * replacement. It makes use of a \ref ResourceLimitedContainerConfig to setup the
 * allocation behaviour regarding the number of elements in the collection.
 *
 * It features linear increment of the capacity, initial preallocation, and maximum number
 * of elements control.
 *
 * @tparam _Ty                 Element type.
 * @tparam _KeepOrderEnabler   Indicates if element order should be kept when removing
 * items, defaults to std::false_type.
 * @tparam _LimitsConfig       Type defining the resource limits configuration,
 *                             defaults to ResourceLimitedContainerConfig
 * @tparam _Alloc              Allocator to use on the underlying collection type,
 * defaults to std::allocator<_Ty>.
 * @tparam _Collection         Type used to store the collection of items, defaults to
...
class ResourceLimitedVector
{
  public:
     ...
     *
     * This constructor receives a \ref ResourceLimitedContainerConfig to setup the
     * allocation behaviour regarding the number of elements in the collection.
     *
     * The cfg parameter indicates the initial number to be reserved, the maximum number
     * of items allowed, and the capacity increment value.
     *
     ...
     *
     * @return Iterator pointing to the inserted value. end() if insertion couldn't be
     * done due to collection limits.
     */
     ...
     * All iterators may become invalidated if this method returns true.
     *
     * @param pred   Unary function that accepts an element in the range as argument and
     * returns a value convertible to bool. The value returned indicates whether the
     * element is considered a match in the context of this function. The function shall
     * not modify its argument. This can either be a function pointer or a function
     * object.
     *
     * @return true if an element was removed, false otherwise.
     */
     ...
     * sequence. The range used is [first,last), which includes all the elements between
     * first and last, including the element pointed by first but not the element pointed
     * by last. The function template argument InputIterator shall be an input iterator
     * type that points to elements of a type from which value_type objects can be
     * constructed. If the size of this range is greater than the maximum number of
     * elements allowed on the resource limits configuration, the elements exceeding that
     * maximum will be silently discarded.
     */
     ...
     *              Will be truncated if greater than the maximum allowed on the resource
     * limits configuration.
     * @param val   Value to fill the container with.
     *              Each of the n elements in the container will be initialized to a copy
     * of this value.
     */
     ...
     *             The compiler will automatically construct such objects from initializer
     * list declarators. Member type value_type is the type of the elements in the
     * container. If the size of this list is greater than the maximum number of elements
     * allowed on the resource limits configuration, the elements exceeding that maximum
  ...
  protected:
     ...
     * This version doesn't keep the order of insertion, optimizing the number of copies
     * performed.
     *
     ...
     * Removes the element pointed to by it.
     * All iterators may become invalidated.
     * This version keeps the order of insertion, so when removing an item different from
     * the last one, part of the collection will be copied.
     *
     * @param it   Iterator pointing to the item to be removed.
     */
    template <typename Enabler = _KeepOrderEnabler>
