...
#define NITEMS ((NBITS + 31u) / 32u)
  ...
  public:
    ...
    // We don't need to define copy/move constructors/assignment operators as the default
     ...
     * This method resets the range and sets a new value for its base, as long as a
     ...
     * This method implements a sliding window mechanism for changing the base of the
     * range.
     *
     * @param base   New base value to set.
     */
     ...
     *
     * @return the highest value set in the range. If the range is empty, the result is
     * undetermined.
     */
     ...
     *
     * @return the lowest value set in the range. If the range is empty, the result is
     * undetermined.
     */
     ...
     *
     * @return true if the item has been added (i.e. is in the allowed range), false
     * otherwise.
     */
     ...
     *
     * @param num_bits         Upon return, it will contain the number of significant bits
     * in the bitmap.
     * @param bitmap           Upon return, it will contain the current value of the
     * bitmap.
     * @param num_longs_used   Upon return, it will contain the number of valid elements
     * on the returned bitmap.
     */
     ...
     * This method is designed to be used when performing deserialization of a bitmap
     * range.
     *
     * @param num_bits   Number of significant bits in the input bitmap.
     * @param bitmap     Points to the beginning of a uint32_t array holding the input
     * bitmap.
     */
  ...
  private:
    void shift_map_left(uint32_t n_bits)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // Need to iterate forward and take 12 bits from next word (shifting it 20
                // bits). aaaaaaaa bbbbbbbb cccccccc dddddddd bbbbbccc bbbbbbbb cccccccc
                // dddddddd bbbbbccc cccccddd ddddd000 dddddddd bbbbbccc cccccddd ddddd000
                // 00000000
    ...
    void shift_map_right(uint32_t n_bits)
    {
        ...
        else
        {
            ...
            {
                // Example. Shifting 44 bits. Should shift one complete word and 12 bits.
                // Need to iterate backwards and take 12 bits from previous word (shifting
                // it 20 bits). aaaaaaaa bbbbbbbb cccccccc dddddddd aaaaaaaa bbbbbbbb
                // cccccccc bbbccccc aaaaaaaa bbbbbbbb aaabbbbb bbbccccc aaaaaaaa 000aaaaa
                // aaabbbbb bbbccccc 00000000 000aaaaa aaabbbbb bbbccccc
                uint32_t overflow_bits = 32u - n_bits;
                size_t last_index      = NITEMS - 1u;
