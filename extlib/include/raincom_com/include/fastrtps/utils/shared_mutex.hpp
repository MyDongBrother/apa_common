/*
   ...
   implementation thus each platform made its own choices: Windows & Boost defaults to
   PTHREAD_RWLOCK_PREFER_WRITER_NONRECURSIVE_NP. Linux & Mac defaults to
   PTHREAD_RWLOCK_PREFER_READER_NP.
 */
...
// Detect if the shared_lock feature is available
    ...
    __has_include(                                                                       \
        <version>) && !defined(__cpp_lib_shared_mutex) || /* deprecated procedure if the \
                                                             good one is not available*/ \
...
#if defined(__has_include) &&                                                         \
    __has_include(                                                                    \
        <version>) && !defined(__cpp_lib_shared_mutex) || /* allow users to ignore    \
                                                             shared_mutex framework   \
                                                             implementation */ \
    (~USE_THIRDPARTY_SHARED_MUTEX + 1) || /* deprecated procedure if the good one is \
                                              not available*/ \
    ( !(defined(__has_include) && __has_include(<version>)) && \
    !(defined(HAVE_CXX17) && HAVE_CXX17) &&  __cplusplus < 201703 )
...
#else
using shared_mutex = detail::debug_wrapper<
    detail::shared_mutex<detail::shared_mutex_type::PTHREAD_RWLOCK_PREFER_READER_NP>>;
#endif // NDEBUG
