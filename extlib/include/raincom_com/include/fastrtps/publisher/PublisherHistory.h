...
/**
 * Class PublisherHistory, implementing a WriterHistory with support for keyed topics and
 * HistoryQOS. This class is created by the PublisherImpl and should not be used by the
 * user directly.
...
class PublisherHistory : public rtps::WriterHistory
{
  public:
     ...
     */
    bool add_pub_change(
        rtps::CacheChange_t *change, rtps::WriteParams &wparams,
        std::unique_lock<RecursiveTimedMutex> &lock,
        const std::chrono::time_point<std::chrono::steady_clock> &max_blocking_time);
     ...
     * @param lock Lock which should be unlock in case the operation has to wait.
     * @param max_blocking_time Maximum time the operation should be waiting.
     * @return true when the last change of the instance history is acknowleged, false
     * when timeout is reached.
     */
    bool wait_for_acknowledgement_last_change(
        const rtps::InstanceHandle_t &handle, std::unique_lock<RecursiveTimedMutex> &lock,
        const std::chrono::time_point<std::chrono::steady_clock> &max_blocking_time);

  private:
    typedef std::map<rtps::InstanceHandle_t, KeyedChanges> t_m_Inst_Caches;

    //! Map where keys are instance handles and values are vectors of cache changes
    //! associated
     ...
     */
    bool prepare_change(
        rtps::CacheChange_t *change, std::unique_lock<RecursiveTimedMutex> &lock,
        const std::chrono::time_point<std::chrono::steady_clock> &max_blocking_time);
};
