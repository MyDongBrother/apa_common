...
class TypeObjectFactory
{
  ...
  protected:
    ...
    DynamicType_ptr build_dynamic_type(
        TypeDescriptor &descriptor, const TypeObject *object,
    ...
    std::string get_key_from_hash(const DynamicType_ptr annotation_descriptor_type,
                                  const NameHash &hash) const;

    /**
     * @brief Fills the TypeInformation provided with the minimal TypeIdentifiers and its
    ...
    /**
     * @brief Fills the TypeInformation provided with the complete TypeIdentifiers and its
     * complete dependencies if possible, or minimal ones in other case.
     * @param info
     * @param ident
     */
     ...
     * @param identifier
     */
    void fill_complete_minimal_dependant_types(TypeInformation *info,
                                               const TypeIdentifier *identifier) const;
  ...
  public:
    ...
    RTPS_DllAPI const TypeIdentifier *get_type_identifier(const std::string &type_name,
                                                          bool complete = false) const;
    ...
    RTPS_DllAPI const TypeIdentifier *get_sequence_identifier(
        const std::string &type_name, uint32_t bound, bool complete = false);

    RTPS_DllAPI const TypeIdentifier *get_array_identifier(
        const std::string &type_name, const std::vector<uint32_t> &bound,
        bool complete = false);

    RTPS_DllAPI const TypeIdentifier *get_map_identifier(
        const std::string &key_type_name, const std::string &value_type_name,
        uint32_t bound, bool complete = false);
    ...
    /**
     * @brief Returns a TypeIdentifierWithSizeSeq object filled with the dependencies of
     * the given identifiers. If continuation_point isn't empty, then it will skip the
     * first (max_size * continuation_point) dependencies.
     * @param identifiers
    ...
    RTPS_DllAPI TypeIdentifierWithSizeSeq typelookup_get_type_dependencies(
        const TypeIdentifierSeq &identifiers, const OctetSeq &in_continuation_point,
        OctetSeq &out_continuation_point, size_t max_size) const;

    /**
     * @brief Fills the given object with the complete version of the given identifier.
     * If the given identifier was MINIMAL, then it will return the stored COMPLETE
     * identifier pointer. Otherwise, it will return the given identifier address (to make
     * comparision trivial).
     * @param identifier
     * @param object
     * @return
    ...
    RTPS_DllAPI const TypeIdentifier *typelookup_get_type(
        const TypeIdentifier &identifier, TypeObject &object) const;
     ...
     * A user that received a TypeInformation from TypeLookupService that calls this
     * method and returns nullptr, must iterate through the TypeInformation dependencies
     * calling recursively to getTypeDependencies method in its participant (which will
     * call the correspondent method in the TypeLookupService), retrieving the TypeObject
     * correspondent and registering the type into the Factory with a name using the
     * add_type_object method, for each COMPLETE TypeIdentifier received in this way until
     * all the hierarchy is registered, and then, the user may call again this method that
     * should return the TypeObject.
     * @param information
     * @return
     */
