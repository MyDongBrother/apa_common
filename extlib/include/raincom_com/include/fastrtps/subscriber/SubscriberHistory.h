...
class SubscriberHistory : public rtps::ReaderHistory
{
  public:
     ...
     */
    SubscriberHistory(const TopicAttributes &topic_att, fastdds::dds::TopicDataType *type,
     ...
     * @param release defaults to true and hints if the CacheChange_t should return to the
     * pool
    ...
    /**
     * Called when a change is received by the Subscriber. Will add the change to the
     * history.
     ...
     * @param [in,out] it     Iterator pointing to change on input. Will point to next
     * valid change on output.
     * @return True if removed.
     */
    ...
    /**
     * @brief A method to get the next instance handle that will miss the deadline and the
     * time when the deadline will occur
     * @param handle The handle to the instance
  ...
  private:
     ...
     */
    bool find_key(rtps::CacheChange_t *a_change, t_m_Inst_Caches::iterator &map_it);
    ...
    bool deserialize_change(rtps::CacheChange_t *change, uint32_t ownership_strength,
                            void *data, SampleInfo_t *info);
};
