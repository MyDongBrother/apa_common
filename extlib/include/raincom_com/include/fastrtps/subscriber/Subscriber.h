...
class RTPS_DllAPI Subscriber
{
  ...
  public:
     ...
     * ReliabilityQosPolicy.max_blocking_time on SubscriberAttributes defines this period
     * of time.
     */
    bool readNextData(void *sample, SampleInfo_t *info);

    /**
     * @brief Takes next sample from the Subscriber. The sample is removed from the
     * subscriber.
     * @param sample Pointer to the object where you want the sample stored.
     * @param info Pointer to a SampleInfo_t structure that informs you about your sample.
     ...
     * It occurs when the Subscriber received all samples sent by Publishers. In other
