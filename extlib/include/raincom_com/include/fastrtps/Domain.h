...
/**
 * Class Domain, use to interact with the Publisher Subscriber API of the Fast RTPS
 * implementation.
...
class Domain
{
  public:
    ...
    RTPS_DllAPI DEPRECATED static Participant *createParticipant(
        const std::string &participant_profile, ParticipantListener *listen = nullptr);
     ...
     */
    RTPS_DllAPI DEPRECATED static Participant *createParticipant(
        const ParticipantAttributes &att, ParticipantListener *listen = nullptr);
     ...
     */
    RTPS_DllAPI static bool getRegisteredType(Participant *part, const char *typeName,
