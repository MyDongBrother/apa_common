...
class LogMessage
{
  public:
    ...
    static inline bool Loggable(LoggingSeverity sev) { return false; }
    // These are the available logging channels
    //  Debug: Debug console on Windows, otherwise stderr
    static void LogToDebug(LoggingSeverity min_sev) {};
    static LoggingSeverity GetLogToDebug() { return LS_NONE; }
    // Testing against MinLogSeverity allows code to avoid potentially expensive
    // logging operations by pre-checking the logging level.
    static int GetMinLogSeverity() { return LS_NONE; }
#else
    LogMessage(const char *file, int line, LoggingSeverity sev,
               LogErrorContext err_ctx = ERRCTX_NONE, int err = 0,
               const char *module = NULL);
...
void LogMultiline(LoggingSeverity level, const char *label, bool input, const void *data,
                  size_t len, bool hex_mode, LogMultilineState *state);
...
#define LOG_E(sev, ctx, err, ...)                                                        \
    LOG_SEVERITY_PRECONDITION(rtc::sev)                                                  \
    rtc::LogMessage(__FILE__, __LINE__, rtc::sev, rtc::ERRCTX_##ctx, err, ##__VA_ARGS__) \
        .stream()
