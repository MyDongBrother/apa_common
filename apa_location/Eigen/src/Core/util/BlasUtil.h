...
template <typename LhsScalar, typename RhsScalar, typename Index, typename DataMapper,
          int mr, int nr, bool ConjugateLhs = false, bool ConjugateRhs = false>
struct gebp_kernel;
...
template <typename Scalar, typename Index, typename DataMapper, int Pack1, int Pack2,
          typename Packet, int StorageOrder, bool Conjugate = false,
...
template <typename Index, typename LhsScalar, typename LhsMapper, int LhsStorageOrder,
...
class blas_data_mapper<Scalar, Index, StorageOrder, AlignmentType, 1>
{
  public:
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE LinearMapper getLinearMapper(Index i,
                                                                       Index j) const
    {
        return LinearMapper(&operator()(i, j));
    }

    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE VectorMapper getVectorMapper(Index i,
                                                                       Index j) const
    {
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE void scatterPacket(Index i, Index j,
...
class blas_data_mapper
{
  public:
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE blas_data_mapper getSubMapper(Index i,
                                                                        Index j) const
    {
    ...
    // storePacketBlock_helper defines a way to access values inside the PacketBlock, this
    // is essentially required by the Complex types.
...
class const_blas_data_mapper : public blas_data_mapper<const Scalar, Index, StorageOrder>
{
  public:
    ...
    EIGEN_ALWAYS_INLINE const_blas_data_mapper<Scalar, Index, StorageOrder> getSubMapper(
...
struct blas_traits<Transpose<NestedXpr>> : blas_traits<NestedXpr>
{
    ...
    typedef Transpose<const typename Base::_ExtractType>
        ExtractType; // const to get rid of a compile error; anyway blas traits are only
                     // used on the RHS
...
struct combine_scalar_factors_impl
{
    ...
    EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE static ResScalar run(const ResScalar &alpha,
                                                               const Lhs &lhs,
                                                               const Rhs &rhs)
    {
...
template <typename ResScalar, typename Lhs, typename Rhs>
EIGEN_DEVICE_FUNC EIGEN_ALWAYS_INLINE ResScalar combine_scalar_factors(const Lhs &lhs,
                                                                       const Rhs &rhs)
{
