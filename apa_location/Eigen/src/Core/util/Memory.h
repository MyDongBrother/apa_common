...
/** \internal Allocates \a size bytes. The returned pointer is guaranteed to have 16 or 32
 * bytes alignment depending on the requirements. On allocation error, the returned
 ...
 * have 16 bytes alignment. On allocation error, the returned pointer is undefined, but a
 * std::bad_alloc is thrown. The default constructor of T is called.
 */
template <typename T>
...
template <typename T, bool Align>
EIGEN_DEVICE_FUNC inline T *conditional_aligned_realloc_new(T *pts, std::size_t new_size,
                                                            std::size_t old_size)
{
 ...
 * requested alignment is typically 16-bytes. If packet size for the given scalar type is
 * 1, then everything is considered well-aligned.
 *
 ...
 * that the array address is a multiple of sizeof(Scalar), as that fails for example with
 * Scalar=double on certain 32-bit platforms, see bug #79.
 *
...
EIGEN_DEVICE_FUNC inline Index first_aligned(const Scalar *array, Index size)
{
    ...
    if (AlignmentSize <= 1)
    {
        // Either the requested alignment if smaller than a scalar, or it exactly match a
        // 1 scalar so that all elements of the array have the same alignment.
        return 0;
    }
    else if ((UIntPtr(array) & (sizeof(Scalar) - 1)) || (Alignment % ScalarSize) != 0)
    {
        // The array is not aligned to the size of a single scalar, or the requested
        // alignment is not a multiple of the scalar size. Consequently, no element of the
        // array is well aligned.
        return size;
    }
    else
    {
...
/** \internal Returns the smallest integer multiple of \a base and greater or equal to \a
 * size
 */
...
class aligned_stack_memory_handler : noncopyable
{
  public:
     ...
     * This constructor takes care of constructing/initializing the elements of the buffer
     *if required by the scalar type T (see NumTraits<T>::RequireInitialization). In this
     *case, the buffer elements will also be destructed when this handler will be
     *destructed. Finally, if \a dealloc is true, then the pointer \a ptr is freed.
     **/
    EIGEN_DEVICE_FUNC
...
struct local_nested_eval_wrapper<Xpr, NbEvaluations, true>
{
    ...
    local_nested_eval_wrapper(const Xpr &xpr, Scalar *ptr)
        : object(ptr == 0 ? reinterpret_cast<Scalar *>(Eigen::internal::aligned_malloc(
 ...
 * scope of this declaration. If BUFFER is non null, then the declared variable is simply
 * an alias for BUFFER, and no allocation/deletion occurs. Here is an example: \code
 * {
 ...
 * \endcode
 * with the advantage of using aligned stack allocation even if the maximal size of XPR at
 * compile time is unknown. This is accomplished through alloca if this later is supported
 * and if the required number of bytes is below EIGEN_STACK_ALLOCATION_LIMIT.
 */
#ifdef EIGEN_ALLOCA

#if EIGEN_DEFAULT_ALIGN_BYTES > 0
  // We always manually re-align the result of EIGEN_ALLOCA.
// If alloca is already aligned, the compiler should be smart enough to optimize away the
// re-alignment.
...
#define ei_declare_aligned_stack_constructed_variable(TYPE, NAME, SIZE, BUFFER)         \
    ...
    Eigen::internal::aligned_stack_memory_handler<TYPE> EIGEN_CAT(                      \
        NAME,                                                                           \
        _stack_memory_destructor)((BUFFER) == 0 ? NAME : 0, SIZE,                       \
                                  sizeof(TYPE) * SIZE > EIGEN_STACK_ALLOCATION_LIMIT)
...
// Case for x64 with PIC. In theory this is only a problem with recent gcc and with medium
// or large code model, not with the default small code model. However, we cannot detect
// which code model is used, and the xchg overhead is negligible anyway.
#define EIGEN_CPUID(abcd, func, id)                                                    \
