...
{
    return svreinterpret_s32_u32(svlsr_u32_z(svptrue_b32(), svreinterpret_u32_s32(a),
...
EIGEN_STRONG_INLINE PacketXi ploaddup<PacketXi>(const numext::int32_t *from)
{
    ...
    indices =
        svzip1_u32(indices, indices); // index in the format {a0, a0, a1, a1, a2, a2, ...}
...
template <>
EIGEN_DEVICE_FUNC inline void pscatter<numext::int32_t, PacketXi>(numext::int32_t *to,
                                                                  const PacketXi &from,
                                                                  Index stride)
{
    // Indice format: {base=0, base+stride, base+stride*2, base+stride*3, ...}
...
EIGEN_STRONG_INLINE numext::int32_t predux_mul<PacketXi>(const PacketXi &a)
{
    ...
    // Extract the high half of the vector. Depending on the VL more reductions need to be
    // done
...
template <>
EIGEN_DEVICE_FUNC inline void pscatter<float, PacketXf>(float *to, const PacketXf &from,
                                                        Index stride)
{
    // Indice format: {base=0, base+stride, base+stride*2, base+stride*3, ...}
    svint32_t indices = svindex_s32(0, stride);
