 ...
 * - if type T has a member typedef Eigen_BaseClassForSpecializationOfGlobalMathFuncImpl,
 * then global_math_functions_filtering_base<T>::type is a typedef for it.
 * - otherwise, global_math_functions_filtering_base<T>::type is a typedef for T.
 *
 * How it's used:
 * To allow to defined the global math functions (like sin...) in certain cases, like the
 * Array expressions. When you do sin(array1+array2), the object array1+array2 has a
 * complicated expression type, all what you want to know is that it inherits ArrayBase.
 * So we implement a partial specialization of sin_impl for ArrayBase<Derived>. So we must
 * make sure to use sin_impl<ArrayBase<Derived> > and not sin_impl<Derived>, otherwise our
 * partial specialization won't be used. How does sin know that? That's exactly what
...
template <typename Scalar, bool HasStdImpl = NumTraits<Scalar>::IsComplex ||
...
struct meta_floor_log2<n, lower, upper, meta_floor_log2_move_up>
{
    enum
    {
        value = meta_floor_log2<n, meta_floor_log2_selector<n, lower, upper>::middle,
                                upper>::value
    };
};
...
struct random_default_impl<Scalar, false, true>
{
    static inline Scalar run(const Scalar &x, const Scalar &y)
    {
        ...
        // complement and have the same number of padding bits in signed and unsigned
        // variants. This is the case in most modern implementations of C++.
...
EIGEN_DEVICE_FUNC inline
    typename internal::add_const_on_value_type<EIGEN_MATHFUNC_RETVAL(real_ref,
                                                                     Scalar)>::type
...
template <typename Scalar>
EIGEN_DEVICE_FUNC inline
    typename internal::add_const_on_value_type<EIGEN_MATHFUNC_RETVAL(imag_ref,
                                                                     Scalar)>::type
...
template <typename Scalar>
struct scalar_fuzzy_impl : scalar_fuzzy_default_impl<Scalar, NumTraits<Scalar>::IsComplex,
                                                     NumTraits<Scalar>::IsInteger>
{
};
...
{
    return scalar_fuzzy_impl<Scalar>::template isMuchSmallerThan<OtherScalar>(x, y,
                                                                              precision);
}
...
struct scalar_fuzzy_impl<bool>
{
    ...
    template <typename OtherScalar>
    EIGEN_DEVICE_FUNC static inline bool isMuchSmallerThan(const bool &x, const bool &,
                                                           const bool &)
    {
        return !x;
    }
