...
{
    typedef typename ScalarBinaryOpTraits<LhsScalar, RhsScalar, scalar_sum_op>::ReturnType
        result_type;
...
#endif
    EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE result_type operator()(const LhsScalar &a,
 ...
 *
 * This is a short cut for conj(x) * y which is needed for optimization purpose; in Eigen2
 * support mode, this becomes x * conj(y)
 */
template <typename LhsScalar, typename RhsScalar>
...
{
    typedef typename ScalarBinaryOpTraits<LhsScalar, RhsScalar, scalar_min_op>::ReturnType
        result_type;
...
{
    typedef typename ScalarBinaryOpTraits<LhsScalar, RhsScalar, scalar_max_op>::ReturnType
        result_type;
...
// The following two classes permits to turn any binary functor into a unary one with one
// argument bound to a constant value. They are analogues to std::binder1st/binder2nd but
