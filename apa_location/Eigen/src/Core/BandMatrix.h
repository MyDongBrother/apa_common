  ...
  public:
    ...
    inline Block<CoefficientsType, Dynamic, 1> col(Index i)
    {
        ...
        if (i <= supers())
        {
            start = supers() - i;
            len =
                (std::min)(rows(), std::max<Index>(0, coeffs().rows() - (supers() - i)));
        }
        else if (i >= rows() - subs())
 ...
 * #SelfAdjoint The former controls \ref TopicStorageOrders "storage order", and defaults
 * to column-major. The latter controls whether the matrix represents a selfadjoint matrix
 * in which case either Supers of Subs have to be null.
 *
...
template <typename _CoefficientsType, int _Rows, int _Cols, int _Supers, int _Subs,
          int _Options>
