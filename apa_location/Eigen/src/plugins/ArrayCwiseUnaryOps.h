 ...
 *
 * This function computes the coefficient-wise exponential. The function MatrixBase::exp()
 * in the unsupported module MatrixFunctions computes the matrix exponential.
 *
 ...
 * however, with finite precision, this function is much more accurate when \c x is close
 * to zero.
 *
 ...
 * MatrixBase::sqrt() in the unsupported module MatrixFunctions computes the matrix square
 * root.
 *
 ...
 */
EIGEN_DEVICE_FUNC
inline const SignReturnType sign_wpy() const { return SignReturnType(derived()); }
 ...
 *
 * \sa <a href="group__CoeffwiseMathFunctions.html#cwisetable_squareE">Math functions</a>,
 * abs2(), cube(), pow()
 */
EIGEN_DEVICE_FUNC
 ...
 * support other scalar types, or float/double in non c++11 mode, the user has to provide
 * implementations of lgamma(T) for any scalar type T to be supported.
 *
 * \sa <a href="group__CoeffwiseMathFunctions.html#cwisetable_lgamma">Math functions</a>,
 * digamma()
 */
EIGEN_DEVICE_FUNC
 ...
 * scalar types, the user has to provide implementations of digamma(T) for any scalar type
 * T to be supported.
 *
 * \sa <a href="group__CoeffwiseMathFunctions.html#cwisetable_digamma">Math functions</a>,
 * Eigen::digamma(), Eigen::polygamma(), lgamma()
 */
EIGEN_DEVICE_FUNC
 ...
 * \note This function supports only float and double scalar types in c++11 mode. To
 * support other scalar types, or float/double in non c++11 mode, the user has to provide
 * implementations of erf(T) for any scalar type T to be supported.
 *
 ...
 * \note This function supports only float and double scalar types in c++11 mode. To
 * support other scalar types, or float/double in non c++11 mode, the user has to provide
 * implementations of erfc(T) for any scalar type T to be supported.
 *
 ...
 * area under the Gaussian probability density function (integrated from minus infinity to
 * x) is equal to y.
 *
 * \note This function supports only float and double scalar types. To support other
 * scalar types, the user has to provide implementations of ndtri(T) for any scalar type T
 * to be supported.
 *
