...
void CompleteOrthogonalDecomposition<MatrixType>::applyZOnTheLeftInPlace(Rhs &rhs) const
{
    ...
    for (Index k = rank - 1; k >= 0; --k)
    {
        ...
        rhs.middleRows(rank - 1, cols - rank + 1)
            .applyHouseholderOnTheLeft(matrixQTZ()
                                           .row(k)
                                           .tail(cols - rank)
                                           .transpose()
    ...
    Rhs &rhs) const
{
    ...
    for (Index k = 0; k < rank; ++k)
    {
        ...
        rhs.middleRows(rank - 1, cols - rank + 1)
            .applyHouseholderOnTheLeft(matrixQTZ().row(k).tail(cols - rank).adjoint(),
                                       zCoeffs()(k), &temp(0));
        if (k != rank - 1)
        {
            rhs.row(k).swap(rhs.row(rank - 1));
...
template <typename _MatrixType>
template <typename RhsType, typename DstType>
void CompleteOrthogonalDecomposition<_MatrixType>::_solve_impl(const RhsType &rhs,
                                                               DstType &dst) const
{
    const Index rank = this->rank();
    if (rank == 0)
