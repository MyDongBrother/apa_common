...
template <typename _StorageIndex>
inline int cm_factorize_p(cholmod_sparse *A, double beta[2], _StorageIndex *fset,
...
class CholmodBase : public SparseSolverBase<Derived>
{
  ...
  public:
    ...
    void factorize(const MatrixType &matrix)
    {
        ...
        cholmod_sparse A = viewAsCholmod(matrix.template selfadjointView<UpLo>());
        internal::cm_factorize_p<StorageIndex>(&A, m_shiftOffset, 0, 0, m_cholmodFactor,
                                               m_cholmod);
    ...
    /** Returns a reference to the Cholmod's configuration structure to get a full control
     * over the performed operations. See the Cholmod user guide for details. */
    ...
    void _solve_impl(const MatrixBase<Rhs> &b, MatrixBase<Dest> &dest) const
    {
        ...
        // etc.) NOTE Actually, the copy can be avoided by calling cholmod_solve2 instead
        // of cholmod_solve
                     ...
                     SparseMatrixBase<DestDerived> &dest) const
    {
        ...
        // TODO optimize this copy by swapping when possible (be careful with alignment,
        // etc.) NOTE cholmod_spsolve in fact just calls the dense solver for blocks of 4
 ...
 * Cholesky factorization using the Cholmod library. This simplicial variant is equivalent
 * to Eigen's built-in SimplicialLLT class. Therefore, it has little practical interest.
 ...
 * Cholesky factorization using the Cholmod library. This simplicial variant is equivalent
 * to Eigen's built-in SimplicialLDLT class. Therefore, it has little practical interest.
 * The sparse matrix A must be selfadjoint and positive definite. The vectors or matrices
 * X and B can be either dense or sparse.
 *
 * \tparam _MatrixType the type of the sparse matrix A, it must be a SparseMatrix<>
 ...
 * Cholesky factorization using the Cholmod library. This supernodal variant performs best
 * on dense enough problems, e.g., 3D FEM, or very high order 2D FEM. The sparse matrix A
 * must be selfadjoint and positive definite. The vectors or matrices X and B can be
 ...
 * selfadjoint and positive definite. The vectors or matrices X and B can be either dense
 * or sparse.
 *
