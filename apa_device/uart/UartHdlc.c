 ...
 * @file UartHdlc.c
 * @brief
 * @see https://github.com/bang-olufsen/yahdlc
...
yahdlc_control_t yahdlc_get_control_type(unsigned char control)
{
    ...
    if (control & (1 << YAHDLC_CONTROL_S_OR_U_FRAME_BIT))
    {
        ...
        {
            // Assume it is an NACK since Receive Not Ready, Selective Reject and U-frames
            // are not supported
    ...
    {
        // It must be an I-frame so add the send sequence number (receive sequence number
        // is not used)
...
void yahdlc_get_data_reset() { yahdlc_get_data_reset_with_state(&yahdlc_state); }
...
{
    return yahdlc_get_data_with_state(&yahdlc_state, control, src, src_len, dest,
                                      dest_len);
}
                               ...
                               unsigned int *dest_len)
{
    ...
    for (i = 0; i < src_len; i++)
    {
        ...
        else
        {
            ...
            if (src[i] == YAHDLC_FLAG_SEQUENCE)
            {
                // Check if an additional flag sequence byte is present or earlier
                // received
                if (((i < (src_len - 1)) && (src[i + 1] == YAHDLC_FLAG_SEQUENCE)) ||
    ...
    else
    {
        ...
        {
            // Return FCS error and indicate that data up to end flag sequence in buffer
            // should be discarded
            *dest_len = i;
            ret       = -EIO;
        }
