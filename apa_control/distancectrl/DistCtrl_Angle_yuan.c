                        ...
                        const int isInit)
{
    ...
    // {
    //     omgn = 7.0;
    //     steerSpd = rte_min(EspSPD, 400);
    // }
    // else if (fabs(curSpd) < 0.3)
    // {
    //     omgn = 8.0;
    //     steerSpd = rte_min(EspSPD, 550);
    // }
    ...
    if (fabs(targetAngle - SAS_SteeringAngle) < 5) // 20240420测试效果：导致最大角度可能超过450，方向盘转动不正常，
                                                   // 尝试：加入最大角度限制
    {
        InitVarFunc();
